...
### Corrigido
- `R/mod_mapping.R:625-639`: observer de sincronia `input[[map_<term>]]` -> `rv$map_values[[term]]` deixava o valor antigo intacto quando o usuario limpava o campo (cliente devolve `input = NULL`, nao `""`). Resultado: termos mapeados via automap (especialmente via alias) persistiam no export mesmo apos o usuario apagar a selecao no card. Fix: distinguir `is.null(input)` na carga inicial (no-op, `rv$map_values` ja era vazio) de `is.null(input)` apos clear (tratar como `""` e prosseguir para o caminho de `build_manual_meta(has_value = FALSE)`).
- `R/utils_export.R:18-21 + 102-117`: `apply_name_review_payload()` injetava `validacao_manual` e `motivo_revisao` em todo export, mesmo sem revisao manual feita. Politica revertida (ADR-088 reverte ADR-051): essas duas colunas nunca mais aparecem no `.csv`/`.xlsx`. Correcoes de `scientificName` (replacement quando usuario marca "Corrigir") **continuam** sendo aplicadas silenciosamente; rastreabilidade fica no `mapping_guide.txt` do bundle (ADR-087).

### Testes
- `tests/testthat/test-mod-mapping-server.R`: novo teste pinning Bug A — `session$setInputs(map_type = NULL)` apos selecao previa zera `rv$map_values$type` e marca `rv$map_meta$type$status = "MANUAL"`. Usa `withr::local_envvar(SAIRA_USER = ...)` para isolar do `rostrum.sqlite` local.
- `tests/testthat/test-utils-export.R`: 5 testes de `apply_name_review_payload()` reescritos para assertar **ausencia** de `validacao_manual` / `motivo_revisao` no output (antes assertavam presenca). Invariantes do replacement de `scientificName` (correcao para multiplas ocorrencias, fallback para original_name quando corrected esta vazio) preservados.

### Documentacao
- `docs/DECISIONS.md`: ADR-088 — reverte parcialmente ADR-051 removendo as colunas de auditoria do export. Mantem o replacement de nomes; rastreabilidade move para o `mapping_guide.txt`.
- `docs/LESSONS.md:141`: bullet antigo ("audit cols sempre") substituido pelo oposto (nunca emitir audit cols, rastreabilidade no mapping_guide.txt). Novo bullet documentando o gotcha de `input[[id]] == NULL` apos clear de `selectInput` (vs NULL na carga inicial).
...
### Adicionado
- `R/utils_export.R`: `process_for_export_with_unmapped(df_processed, raw_data, map_values)` — envolve `process_for_export()` e faz `cbind` das colunas brutas que NAO foram source de nenhum mapping ao final do data frame. Antes essas colunas desapareciam do CSV exportado; agora sao preservadas no fim para o usuario poder editar manualmente apos o upload no IPT (ADR-087).
- `R/utils_export.R`: `write_xlsx_text_only(df, path)` — escreve `.xlsx` via `writexl::write_xlsx()` com TODAS as celulas coercidas a `character` antes da escrita. Sobrevive duplo-clique no Excel sem corromper datas ISO (`2024-01-15` em vez de `15/01/2024`), zeros a esquerda ou notacao cientifica em numeros grandes (ADR-087).
- `R/utils_export.R`: `build_mapping_guide_txt(map_values, raw_data, lang, ...)` — texto plano dual-purpose com header magico `# saira:mapping:v1`, instrucoes bilingue PT/EN, lista pares `Coluna -> Termo`, lista termos DwC obrigatorios faltantes e lista colunas brutas nao-usadas. **Sem dados embutidos** — apenas vocabulario de mapping (PII-safe) (ADR-087).
- `R/utils_rostrum_templates.R`: `is_saira_mapping_guide(path)` (detecta header magico lendo so linha 1, safe para CSVs grandes); `parse_mapping_guide_txt(path)` (extrai metadata das linhas `#` e pares `source -> term` do corpo, ignora silenciosamente bare lines sem `->`, warn so em linhas com `->` malformadas); `import_mapping_guide_to_aliases(payload, conn = NULL, scope = "personal", user_id = NULL)` (chama `rostrum_upsert_alias()` por par com `confidence = 1.0`, `reviewed = TRUE`; idempotente — re-importar nao duplica; composicoes `colA + colB` viram aliases independentes) (ADR-087).
- `R/mod_preview.R`: `downloadHandler` agora emite `dwc_export_<data>.zip` com 3 arquivos (CSV IPT-ready, XLSX Excel-safe, mapping_guide.txt) via `zip::zipr()`. Modal de loading ganha botao "Cancelar" explicito em PT/EN que reseta `is_exporting(FALSE)` para destravar a UI sem violar `easyClose = FALSE` (ADR-009 mantido). Error fallback embrulha CSV-de-erro em `.zip` para nome de arquivo bater com extensao (ADR-087).
- `R/mod_mapping.R`: 4o botao "Importar modelo" na sidebar (`btn-outline-secondary`, agrupado com "Adicionar termo" como acao auxiliar). Abre modal com `fileInput` para `.txt`; ao confirmar, valida magic header, parseia e popula `rostrum_aliases` via `import_mapping_guide_to_aliases()`. Slot novo `map_values_r = shiny::reactive(rv$map_values)` no list de retorno do `mod_mapping_server` (ADR-054 estendido) — consumido por `mod_preview_server` para gerar o `mapping_guide.txt` no export (ADR-087).
- `R/mod_upload.R`: `fileInput` aceita `.txt` alem de `.csv`; novo reactive `file_kind()` classifica upload em `data`/`guide`/`invalid` via `is_saira_mapping_guide()`. Quando guide, abre modal de confirmacao com contagem de pares e botao "Importar"; ao confirmar, chama `import_mapping_guide_to_aliases()` e mantem `raw_data()` como `NULL` para nao confundir com dados (ADR-087).
- `R/utils_mapping.R`: `map_occurrence_status_values()` — converte representacoes binarias de presenca/ausencia (`0/1`, `sim/nao`, `yes/no`, `presente/ausente`, `present/absent`, `TRUE/FALSE`) para os literais DwC `present` / `absent` na exportacao. Convencao: `0 = absent`, `1 = present`. Valores nao reconhecidos passam direto. Branch dedicado em `build_processed_mapping_df()` (ao lado de `basisOfRecord`) aciona a transformacao quando o usuario mapeia `occurrenceStatus`.
- `inst/extdata/dwc_terms.rds`: termo `occurrenceStatus` (class: Occurrence) adicionado ao conjunto-base (50 -> 51 termos), passa a aparecer nativamente no card de mapeamento sem precisar ser inserido como termo extra. Patch reproduzivel em `data-raw/add_occurrence_status_to_base.R` (idempotente).
- `R/utils_export.R`: `dwc_canonical_class_order()`, `dwc_canonical_preferred_terms()` e `order_columns_dwc_canonical()` — ordem canonica para colunas exportadas/preview, lendo a `class` de cada termo via `get_dwc_full_catalog()` (217 termos) e ordenando por (prioridade de classe, termo preferido dentro da classe, posicao no catalogo, posicao original). Resultado: `occurrenceID` lidera a planilha, bloco Taxon comeca com `scientificName` seguido da hierarquia (kingdom -> genus -> specificEpithet -> taxonRank), `decimalLatitude/Longitude` lideram Location. Termos extras adicionados na sessao caem automaticamente no bloco semantico correto; colunas nao-DwC sao preservadas no fim.
- `R/utils_mapping.R`: handler dedicado para `dynamicProperties` que compoe duas ou mais colunas do CSV em JSON estrito TDWG por linha (`{"chave1":"valor1","chave2":"valor2"}`, sem espacos, com escape JSON correto de `"`, `\\`, controles e UTF-8 multi-byte preservado). Funcoes puras vetorizadas: `derive_dynprops_key()` (lowercase + `iconv ASCII//TRANSLIT` + `[^a-z0-9]+` -> `_`, fallback `field`), `json_escape_string()` com fast-path quando nao ha caracter especial, e `build_dynamic_properties_json(df, cols, keys)` que retorna `character(nrow(df))`. Linhas com todas as colunas vazias produzem `""` (nao `"{}"`); chaves duplicadas apos normalizacao geram um unico `warning()` e a primeira coluna vence (demais sao descartadas).
- `R/mod_mapping_cards.R`: branch dedicado para `dynamicProperties` em `build_field_card()` — alem do `selectInput` multi-coluna padrao, exibe um bloco `.dynprops-keys-block` com um `textInput` por coluna selecionada, mostrando a chave auto-derivada como placeholder e permitindo override por usuario.
- `R/mod_mapping.R`: novo slot `rv$dyn_props_keys` (named list `coluna -> chave override`), observer reativo que sincroniza os inputs `dynprops_key_*` em `rv$dyn_props_keys`, propagacao via `build_mapped_result -> build_processed_mapping_df(dyn_props_keys = ...)`, e reset junto com o restante do estado de mapeamento quando `raw_data_r()` muda.
- `R/mod_mapping.R`: modal "Adicionar termo" passa a renderizar termos agrupados por classe DwC via `<optgroup>` nativo do Shiny (`choices = list(grupo = c(...))`), na ordem canonica TDWG (`Record-level, Occurrence, Organism, MaterialEntity, MaterialSample, Event, Location, GeologicalContext, Identification, Taxon, MeasurementOrFact, ResourceRelationship`), termos alfabeticos dentro de cada grupo. Modal usa `size = "l"` para acomodar a lista. Os labels dos `<optgroup>` reutilizam `category_label()` ja existente para tradução PT/EN.
- `inst/extdata/i18n.json`: 3 chaves novas para o card de `dynamicProperties` (`dynprops_keys_header`, `dynprops_help`, `dynprops_key_placeholder` com formato `sprintf` para a auto-key); placeholder do modal "Adicionar termo" (`modal_add_term_placeholder`) atualizado para refletir o novo agrupamento.
- `inst/app/www/css/14-mapping.css`: bloco `.dynprops-keys-block` com borda esquerda em `var(--accent)`, fonte mono nas chaves e na origem da coluna, e tipografia compativel com o design system; bundle regenerado via `data-raw/build_css.R`. Sem novos `!important` (contagem permanece em 11/13).
- `inst/extdata/dwc_full_catalog.rds`: catalogo completo dos termos recomendados do TDWG (`dwc:` + `dcterms:`, ~217 termos) gerado por `data-raw/build_dwc_full_catalog.R`. Schema identico ao de `dwc_terms.rds`; deduplicacao por prioridade de namespace (`dwc:` > `dcterms:` > `dc:`); termos-base aparecem primeiro, extras ordenados alfabeticamente (ADR-081).
- `R/utils_dwc.R`: `get_dwc_full_catalog()` (cache em memoria via `create_rds_cache`), `get_active_dwc_terms(extra)` e `get_active_dwc_terms_list(extra, lang)` para combinar conjunto-base com extras adicionados sob demanda na sessao, mantendo o schema de `get_dwc_terms()`/`get_dwc_terms_list()` para retrocompatibilidade dos consumidores (ADR-082).
- `R/mod_mapping.R`: botao "Adicionar termo" na sidebar abre modal com `selectizeInput` populado pelo catalogo completo (menos termos ja ativos); `confirm_add_term` insere em `rv$extra_terms`; `confirm_reset` limpa o estado para retornar ao conjunto-base (ADR-082).
- `R/utils_rostrum_templates.R`: `rostrum_extra_terms_from_template()` identifica quais termos de um payload de template precisam ser pre-ativados antes da aplicacao — funcao pura testavel para fluxos futuros de aplicacao de template via UI (ADR-082).
- 6 chaves novas em `inst/extdata/i18n.json` para o botao Adicionar termo (`btn_add_term`, `modal_add_term_title`, `modal_add_term_label`, `modal_add_term_placeholder`, `btn_confirm_add_term`, `notif_term_added`) e 6 chaves para as classes DwC adicionais (`class_geologicalcontext`, `class_materialentity`, `class_materialsample`, `class_measurementorfact`, `class_organism`, `class_resourcerelationship`).
- `inst/extdata/i18n.json`: 15 chaves novas PT/EN para o bundle ZIP + round-trip de aliases (ADR-087): `preview_export_cancel`, `preview_export_cancelled`, `upload_guide_detected_title`, `upload_guide_detected_message`, `upload_guide_import_btn`, `upload_guide_cancel_btn`, `upload_guide_success`, `upload_guide_invalid`, `upload_guide_failed`, `btn_import_template`, `modal_import_template_title`, `modal_import_template_help`, `modal_import_template_label`, `modal_import_template_confirm`, `modal_import_template_no_file`, `modal_import_template_invalid_magic`, `modal_import_template_success`.
...
### Alterado
- `R/mod_preview.R`: assinatura do `mod_preview_server()` ganha parametros opcionais `raw_data_r` e `map_values_r` (ambos `NULL` por padrao para retrocompat). Quando passados, sao consumidos APENAS dentro do `content` callback do `downloadHandler` (nao-reativo) — sem dependencias reativas espurias. `R/app_server.R`: wiring atualizado para passar `raw_data` e `mapping_result$map_values_r`.
- `R/mod_preview.R`: `shiny::outputOptions(output, "download_real", suspendWhenHidden = FALSE)` adicionado apos o `downloadHandler` para forcar o envio do URL ao cliente mesmo quando o `downloadButton` esta dentro de wrapper `display:none;`. Padrao ja existente em `mod_mapping.R:553` para `file_uploaded`. Veja entrada em "Corrigido" e LESSONS.md (ADR-087).
- `R/utils_rostrum_engine.R`: `rostrum_apply_alias_overrides()` agora promove `user_id = ""` -> `"anonymous"` antes de chamar `rostrum_lookup_alias()`, espelhando a promocao que `rostrum_upsert_alias()` ja faz na insercao (`R/utils_rostrum_db.R:470-472`). Veja entrada em "Corrigido" (ADR-087).
- `DESCRIPTION`: novas dependencias `writexl (>= 1.4.0)` e `zip (>= 2.3.0)` em `Imports`. `renv.lock` sincronizado.
- `docs/ENCODING_RULES.md`: regra 4 estendida — bundle de export pode incluir `.xlsx` companheiro via `writexl::write_xlsx()` com todas celulas coercidas a `character` antes da escrita; CSV principal continua UTF-8 sem BOM (ADR-087).
- `R/utils_export.R`: `process_for_export()` agora chama `order_columns_dwc_canonical()` como ultima etapa, garantindo que o CSV baixado siga a ordem canonica DwC. `R/utils_preview.R`: `prepare_preview_data()` aplica a mesma reordenacao para que a tabela do preview espelhe o que sera exportado, evitando dissonancia preview vs download.
- `R/utils_mapping.R`: `build_processed_mapping_df()` ganha parametro `dyn_props_keys = list()` e despacha `dynamicProperties` para o handler JSON dedicado **antes** da branch single-column generica. Consequencia: dynamicProperties com **uma unica** coluna mapeada agora produz `{"coluna":"valor"}` em vez do valor cru. Justificativa: emitir valor cru no campo dynamicProperties ja era invalido por TDWG; e correção semantica, nao regressao funcional. Templates Rostrum salvos com dynamicProperties single-column continuam carregando — apos aplicacao, o handler aplica o formato JSON automaticamente. Templates ainda nao persistem `dyn_props_keys` (out of scope desta entrega; auto-derivacao fica em vigor ate aplicacao manual).
- `R/mod_wiki.R`: aba Wiki passa a usar `get_dwc_full_catalog()` (217 termos / 12 classes) em vez do conjunto-base de 50 termos; coluna de exemplos removida (ADR-083); `applyPageLength` corrigido para `table.page.len(len).draw(false)` para recalcular paginacao corretamente apos mudanca de tamanho de pagina; `class_tr_keys` e `classSlugMap` (JS) extendidos para cobrir todas as 12 classes do catalogo, com `classSlugMap` agora gerado dinamicamente em R e injetado via `jsonlite::toJSON` para evitar drift no futuro (ADR-081/083).
- `R/mod_mapping.R`: `dwc_all` reactive passa a chamar `get_active_dwc_terms_list(extra = rv$extra_terms, lang = lang_r())`; `all_filter_categories` migrou de variavel hardcoded para reactive derivado de `unique(dwc_all()$category)`; auto-map degrada `AUTO -> SUGERIDO` para termos extras (nao calibrados pelo Rostrum); `class_tr_keys_map` extendido para as 12 classes (ADR-082).
- `inst/app/www/css/02-navbar.css`: regra `.nav-title-container:has(.nav-title-dynamic:not(:empty)) .nav-title-static { display: none; }` movida do bundle `custom.css` (onde tinha sido editada diretamente em commit anterior) para o modulo de origem, garantindo que a regra sobreviva a regeneracoes do bundle por `data-raw/build_css.R`. Sem essa regra os titulos de navegacao apareciam duplicados ("Mapeamento Mapeamento", "Coordenadas Coordenadas").
- `inst/app/www/css/08-wiki.css`: 6 regras de `.wiki-filter-pill[data-filter="..."]` (dot + active) e 6 regras de `.wiki-class-badge--{slug}` adicionadas para as classes do catalogo completo, mantendo consistencia visual com o conjunto-base (ADR-081/082).
...
### Corrigido
- **Bug critico de export** (ADR-087): clicar em Baixar produzia o HTML inteiro do app salvo como `dwc_export_<data>.csv`, com modal de progresso travado em 90% e UI congelada. Causa raiz: o `downloadButton` em `mod_preview.R` esta dentro de `<div style="display: none;">` e Shiny suspende outputs ocultos por padrao (`suspendWhenHidden = TRUE`), entao o `<a href>` ficava vazio. Click num `<a href="" download="dwc_export_xxx.csv">` faz o navegador baixar a URL atual (a propria pagina do app) com o nome `.csv`. Endpoint nunca era chamado, logo `download_finish_channel` nao chegava ao cliente, modal travava em 90%, e `is_exporting(FALSE)` (que mora no `finally` do callback nunca-chamado) nao disparava. Fix one-liner: `shiny::outputOptions(output, "download_real", suspendWhenHidden = FALSE)` apos o `downloadHandler`. Modal tambem ganhou botao "Cancelar" como defesa em profundidade.
- **Bug colateral no sistema de aliases** (ADR-087): `rostrum_apply_alias_overrides()` lookav aliases com `user_id = ""` (env `SAIRA_USER` vazia), mas `rostrum_upsert_alias()` promove `""` -> `"anonymous"` ao salvar. Mismatch fazia o filtro de visibilidade em `rostrum_list_aliases_for_column:780` rejeitar todos os aliases personal por `user_id_norm = NA`. Resultado: aliases salvos via `rostrum_record_alias_override` (selecao manual de coluna) OU `import_mapping_guide_to_aliases` (round-trip do .txt) ficavam invisiveis ao motor durante automap em sessoes sem `SAIRA_USER` setado. **Sistema de aliases personal estava quebrado por meses** ate o usuario testar o round-trip e perceber que automap "ignorava" o .txt importado. Fix: `rostrum_apply_alias_overrides()` tambem promove `""` -> `"anonymous"` antes do lookup.
- Wiki e Preview pareciam travadas apos `fac0f7b` ter expandido o catalogo DwC (50 -> 217 termos / 6 -> 12 classes / toolbar com 12 pills). O conteudo das duas abas passou a extrapolar a viewport (em `pageLength` alto na Preview, ou no Wiki com pageLength=25/50/100), mas `bslib::page_navbar(fillable = TRUE)` confina o body em `height: 100vh; overflow: hidden`, escondendo o footer (info + paginacao) abaixo da dobra sem possibilidade de scroll. Correcao em **uma unica regra CSS** em `inst/app/www/css/12-overrides.css` usando `:has(.wiki-module)` e `:has(.preview-page)` para liberar `overflow-y: auto` apenas nessas duas abas, preservando o contrato viewport-bound de Validate Names/Coords (`calc(100vh - 166px)`). Classe marker `preview-page` adicionada ao container de `mod_preview_ui` (a `wiki-module` ja existia). Tabelas voltam ao comportamento v0.2.0: crescem naturalmente com a `pageLength` selecionada, scroll horizontal interno do DT (`scrollX = TRUE`) intacto, paginacao e cabecalho do DT no topo dos registros visiveis. Zero novos `!important` (contagem do bundle volta a 11).
- Titulos de navegacao apareciam duplicados no header e nos submenus de validacao ("Saira Saira", "Mapeamento Mapeamento", "Coordenadas Coordenadas"). A regra CSS que escondia o fallback estatico vivia somente em `inst/app/www/custom.css` (commit `97b7cf7`) e foi perdida na regeneracao do bundle pelo `data-raw/build_css.R`. A regra agora vive em `inst/app/www/css/02-navbar.css` e e protegida por guardrail em `tests/testthat/test-css-guardrails.R`.
...
### Testes
- `tests/testthat/test-utils-mapping.R`: 8 novos testes para `derive_dynprops_key`, `json_escape_string` (inclui controle BEL e UTF-8 multi-byte) e `build_dynamic_properties_json` (single col, multi col, NA por linha, override de chave, fallback para auto quando override invalido, escape de `"` `\\` `\n`, colisao de chave com warning, coluna ausente, integracao via `build_processed_mapping_df` confirmando dispatch antes do single-column branch).
- `tests/testthat/test-mod-mapping-server.R`: 2 novos testes em `testServer` cobrindo (a) override de chave por usuario propaga para `processed_data_r()` em formato JSON e blanking restaura auto-derivacao; (b) reset de `rv$dyn_props_keys` quando `raw_data_r()` muda.
- `tests/testthat/test-performance-regression.R`: novo benchmark `RUN_PERF=true` exigindo `< 0.5s` para `build_dynamic_properties_json` em 100k linhas x 4 colunas (real ~0.2s na maquina de dev).
- `tests/testthat/test-i18n-a11y-keys.R`: 4 chaves novas para `dynamicProperties` (`dynprops_keys_header`, `dynprops_help`, `dynprops_key_placeholder`) e atualizacao do `modal_add_term_placeholder` cobertas pelo teste de presenca em PT/EN.
- `tests/testthat/test-utils-dwc.R`: 7 novos testes cobrindo `get_dwc_full_catalog()` (schema, superset do conjunto-base, preservacao de PT) e `get_active_dwc_terms`/`get_active_dwc_terms_list` (sem extras, com extra valido, dedup com base, extra invalido fora do catalogo).
- `tests/testthat/test-utils-rostrum-templates.R`: 4 testes para `rostrum_extra_terms_from_template()` cobrindo separacao entre termos a ativar e termos desconhecidos.
- `tests/testthat/test-css-guardrails.R`: nova asserção que falha se a regra `nav-title-container:has(.nav-title-dynamic:not(:empty)) .nav-title-static` desaparecer do bundle, prevenindo a regressao de titulos duplicados em futuras reconstrucoes do CSS.
- `tests/testthat/test-i18n-a11y-keys.R`: 12 chaves novas (6 do botao Adicionar termo e 6 das classes DwC sob demanda) cobertas pelo teste de presenca/resolucao em PT/EN.
...
- `tests/testthat/test-utils-export.R`: 7 testes novos para o bundle ZIP (ADR-087) — `process_for_export_with_unmapped()` preserva colunas nao-mapeadas no fim/no-op quando nao ha extras/tolera `NULL`; `write_xlsx_text_only()` produz `.xlsx` round-trip-readable com tipos `character`, datas ISO intactas, zeros a esquerda preservados, numeros grandes nao viram notacao cientifica; `build_mapping_guide_txt()` emite header magico + pares + SEM dados embutidos (anti-PII-leak) + switching PT/EN + flag de obrigatorios faltantes.
- `tests/testthat/test-utils-rostrum-templates.R`: 8 testes novos para o round-trip de aliases (ADR-087) — magic detection, parse com metadata + pares, rejeicao sem magic, skip silencioso de bare lines (colunas nao-mapeadas), warning so em lines com `->` malformado, idempotencia do `import_mapping_guide_to_aliases` (re-importar nao duplica), split de composicoes `colA + colB`, round-trip `build -> writeLines -> parse` preserva pares, end-to-end com `rostrum_lookup_alias`; teste de regressao para o bug colateral de SAIRA_USER (importa com user default e verifica que `run_rostrum_engine` retorna `status = "ALIAS"` com `context = list()`).
- `tests/testthat/test-mod-mapping-server.R`: isolamento via `withr::local_envvar(c(SAIRA_USER = paste0("test_isolation_", as.integer(Sys.time()))))` no teste "v1 auto-map applies metadata and manual override becomes EDITADO" — necessario porque o fix do bug colateral de aliases fez o teste herdar estado do `~/.local/share/saira/rostrum.sqlite` real do dev. Atualizacao do `expect_named` no smoke test para incluir slot `map_values_r`.
...
- `docs/DECISIONS.md`: ADR-086 (dynamicProperties como composicao JSON estrita TDWG, com chave auto-derivada e override por usuario); ADR-078 (mascara `land 10m` embutida); ADR-081 (catalogo DwC completo separado do conjunto-base); ADR-082 (mapeamento sob demanda via `rv$extra_terms`); ADR-083 (remocao da coluna de exemplos da wiki + correcao do seletor de pageLength).
- `docs/LESSONS.md`: novas licoes registradas sobre artefatos espaciais embutidos, timeout opt-in, erro de recortar `land` fisico por fronteira politica, e o conjunto "DwC / Catalogo Sob Demanda (2026-05-06)" cobrindo armadilhas de `[[` em vetor nomeado, deduplicacao por namespace TDWG, separacao entre catalogo e conjunto-base, downgrade de `AUTO -> SUGERIDO` para extras, truncagem de exemplos literais e correcao de `draw('page')` -> `draw(false)`.
- `docs/DECISIONS.md`: ADR-087 — Export como bundle ZIP (CSV IPT-ready + XLSX Excel-safe + mapping_guide.txt) com round-trip via aliases. Cobre: (1) fix do bug critico de export (suspendWhenHidden em downloadButton oculto), (2) bundle ZIP de 3 arquivos com colunas nao-mapeadas preservadas e XLSX text-only para sobreviver duplo-clique no Excel, (3) round-trip de aliases via `mapping_guide.txt` com magic header + parse + populacao de `rostrum_aliases`, (4) bug colateral de mismatch `SAIRA_USER` empty/anonymous entre upsert e lookup.
- `docs/architecture.md`: nova secao 3.2.1 "Bundle ZIP do export" detalhando os 3 arquivos do bundle, a tabela de funcoes geradoras, e o aviso critico sobre `outputOptions(suspendWhenHidden = FALSE)` para `downloadButton` oculto.
- `docs/LESSONS.md`: 3 bullets novos cobrindo (a) `outputOptions(suspendWhenHidden = FALSE)` obrigatorio em outputs ocultos via `display:none;`, (b) mismatch silencioso de promocao `""` -> `"anonymous"` entre `rostrum_upsert_alias` e `rostrum_apply_alias_overrides` que escondia aliases personal por meses, (c) testes de modulo que usam `rostrum_connect()` default herdam estado do `~/.local/share/saira/rostrum.sqlite` real do dev e exigem isolamento por `withr::local_envvar(c(SAIRA_USER = ...))`.
