...
### Changed
- **The mapping screen fits three columns of cards instead of two.** The grid was `repeat(2, 1fr)` at any width, so a 1920px monitor gave each card 755px for a selector that needs 400. It is now three columns, dropping to two below 1280px and one below 860px, which takes the 66 Darwin Core terms from roughly 7.5 screens of scrolling to about 3. Card descriptions are clamped so every card in a row is the same height, and `dynamicProperties` spans two tracks because its key rows are built with `bslib::layout_columns()`, whose breakpoints follow the viewport rather than the card.
- **The card scale follows the monitor.** Up to 1800px a card is about 137px tall: two lines of description, tightened padding and leading. Above it, about 117px: the description drops to one line, because a card is then around 499px wide and 56 of the 66 texts fit on a single line, so the second one was height the card reserved without using. The term name also steps down one size there. No setting to choose, and nothing is hidden either way: the full definition is on the description's tooltip at every width, which covers the ten texts long enough to be cut on a wide screen.
- **Eleven Darwin Core definitions now have a short line written for the card.** Of the 66 base definitions the median is 66 characters, but eleven run past 90 and `coordinateUncertaintyInMeters` reaches 313, which is what broke card height in three columns. Those eleven carry `card_hint_pt`/`card_hint_en` in `dwc_terms.rds`; the official definition is untouched and still shown in full by the `?` next to the term name, the Wiki and the upload requirements panel. The `?` opens a `bslib::tooltip()` rather than a native `title`, which browsers hold back for a second or two and never open on a click. What was dropped from each is the usage qualification, not the meaning.

### Added
- **The mapping screen has a list view, for reviewing what auto-mapping decided.** A Cards / List switch in the sidebar. The list puts all 66 terms on one line each, with the source column and one example value side by side, which is what makes a column that landed on the wrong term visible: two columns swapped between `county` and `locality` read as correct on their own cards, and no existing warning catches them, because each column was used once. Rows are read-only and clicking one returns to that term's card, so the terms with their own block (license, `dynamicProperties`, the fixed-value input, the two assistants) need no special case and nothing loses an editor.
- **The class pill bar now shows where the pending work is.** Each pill carries a state dot, red when the class has a required term with no mapping and amber when it has a term the Rostrum was not sure about, and a "next pending" button at the end of the bar jumps to each one in turn and highlights it. Cards in those two states are marked by border colour and a light fill, following the same grammar as the green border on a mapped card. Nothing is hidden or collapsed: an automatic mapping that landed on the wrong column is exactly what nobody would go looking for.
- **The release cards on the help site are back to the house style.** The 0.9.x cards had grown into paragraphs carrying milliseconds, function names and root causes: v0.9.5 alone ran to 363 words across 7 items, against the 20-word one-liners of v0.5.0 to v0.8.x. Versions 0.9.0 and 0.9.3 to 0.9.7 were rewritten in both languages to at most 4 items of at most 25 words, each saying what the reader now sees in the app. The detail stays here and in the ADRs. `.claude/rules/website.md` records the budget, and the PR template now asks for a one-line summary.
...
- **The GBIF-fit diagram in the README was redrawn to match the how-it-works diagram.** It was the last drawing still in the old language: five white boxes in a 960x280 band with 10.5px labels, sitting directly above the redrawn flow diagram, so the two read as coming from different projects. It is now the same 1000x460 canvas, the same flow curve, the same five circular stations with labels alternating above and below, and the same 14 to 18px type. Because this diagram describes a position in the GBIF pipeline and not a procedure, the slot that carries `01`-`05` in the flow diagram carries the role of each station instead (`ORIGEM`, `PORTÃO DE QUALIDADE`, `REDE`, `A JUSANTE`, `USO FINAL`), and Saíra, the station the diagram exists to place, is ringed with a dashed circle that sets it apart from the four stations around it. The network station reads `SiBBr / GBIF` in the Portuguese drawing and `GBIF / SiBBr` in the English one, so each names the aggregator its readers publish through first. Two content errors were fixed on the way: the sources station listed Excel as an input format, which the rest of the README had already stopped doing, and the downstream station spent its title on its role.
...
### Changed
- **Saira learns a column name only when you export, not when you pick it.** Every manual column selection used to be written to the alias store immediately, so cycling a card through candidates taught it each rejected one: the local database had accumulated `id`, `1`, `class`, `datasetname` and `road id` as learned aliases for `basisOfRecord`, and `especie` pointed at both `scientificName` and `habitat`. A selection is a hypothesis; exporting is the confirmation. The final mapping is now committed once, in a single transaction under one run identifier, so a single export can also be undone as a unit. Terms assembled from several columns, such as an `eventDate` built from year/month/day, are skipped because they are not one-to-one aliases. Importing a mapping template still writes immediately, since that is installing a curated file rather than learning from work in progress.

### Fixed
- **Mapping a column no longer freezes the screen for a minute.** On a 21,500-row dataset whose mapping guide assembles four terms from several columns each, mapping `basisOfRecord` took about 65 seconds to show the assistant button, the Preview sheet never arrived, and the country card only turned green when the other two did. Building the mapped table walked the dataset one row at a time for every term assembled from more than one column, splitting and trimming each cell individually; profiling put 71% of the time in that one loop. It now works a column at a time instead, which is 62 to 89 times faster per term and takes a full rebuild from 8.6 to 1.3 seconds. Output is identical, including for cells holding empty or repeated separators, and a test in the suite re-checks that against the old implementation rather than trusting the claim.
- **The generalization tab stops rebuilding the whole dataset while you are mapping.** Its map overlay, its map framing and its result filter each pulled the full mapped table on every mapping change, from whatever tab you were on, for a map nobody was looking at. Shiny pauses a hidden tab's outputs but never its observers. That work now waits for the tab to be on screen. The generalization decision the export reads is deliberately left ungated, so exporting from another tab still carries it.
- **The test suite no longer writes into your local alias database.** Two tests isolated the user identity but not the database file, so every `devtools::test()` run added two rows to the real `rostrum.sqlite`; 376 of its 566 aliases were test residue. `SAIRA_DATA_DIR` is now redirected to a temporary directory for the whole suite, with a guard test that fails if that isolation is ever dropped.
- **A required mapping card no longer stays red after you fill it.** The red border and fill that mark a required term with no mapping were decided when the card grid was drawn, and the grid is deliberately not redrawn when you pick a column, so the card kept the warning state until something unrelated forced a rebuild. It was worst on `basisOfRecord`, where the red wash sits directly under the assistant button and made it look disabled. The incremental update that already flips the mapped border and the status badge now carries the state as well, in both directions.
- **A fixed value on the country card now releases coordinate validation.** On a single-country dataset whose spreadsheet has no country column, filling the country card with a fixed value is the intended way to resolve the term, and the card turned green when you did. Coordinate validation stayed blocked anyway, reporting country as not mapped. The lightweight gate the coordinate tab reads resolved country only as a source column, so it never saw the fixed value, even though the mapped data really does carry it on every row and the validation itself would have run fine. The gate now accepts a fixed value the same way `is_field_mapped()` already did, and the coordinate card labels it as a value rather than printing it where the latitude and longitude rows print a column name.
- **Importing a mapping template no longer looks like it froze.** Confirming the import left the modal sitting there for a couple of seconds and then closed it onto a screen that kept working silently, which read as the app quietly running an auto-mapping pass over the guide. It is not: the time goes to the alias writes and to a full rebuild of the card grid, which restoring `scientificName` forces because it flips one of the four structural dependencies of the grid. The import now shows the same progress modal the auto-map uses, with its own wording so the two are not confused, and the modal stays up until the rebuilt cards are on screen instead of closing before them. The alias write also reuses the module's SQLite connection rather than opening a second one to the same file, which was re-running the schema migration check and making the two contend for the same write lock.
...
- **Three pieces of work that were computed and then thrown away are no longer computed.** On a spreadsheet whose rows all carry a valid month and year, the four-column eventDate mapping now takes 0.27s instead of 15.1s over 50,000 rows: it was building the "show the raw values instead" fallback text for every row even when no row needed it. When your file already brings its own `occurrenceID` (a camera-trap `observationID`, or data re-imported from GBIF), Saira no longer generates a full set of random identifiers just to discard them, which saves 0.86s per mapping pass at that size. And accepting a transposed-coordinate correction no longer re-resolves every country name to its ISO3 code, including a fuzzy match against roughly 3,000 aliases, seconds after the coordinate validation already resolved exactly that.
- Datasets where many rows fail to parse still build the fallback, because there it is actually displayed. That is the intended behaviour and it is unchanged.
...
### Added
- **`establishmentMeans` and `degreeOfEstablishment` can now be filled for a whole spreadsheet, one answer per species.** Both terms carry a TDWG controlled vocabulary (7 and 11 values), so filling them by hand across thousands of rows was not realistic. They are now default mapping cards, and each offers an assistant that lists the **unique species** found in the column mapped to `scientificName` — with a record count per taxon, ordered by how much data each answer covers — and asks for the two values once per species. The answer is expanded to every record of that taxon. Species on the bundled invasive alien species list arrive pre-filled as `introduced` and are marked with a badge in the assistant. `degreeOfEstablishment` is deliberately never suggested: whether an individual is `captive`, `released` or `invasive` is a property of the record, not of the species. Both selects offer the full controlled vocabulary with the TDWG definition next to each value, in Portuguese and English.
- **Your own data always wins over the assistant.** If you also map a column to either term, the column's values are kept and the assistant only fills the rows you left blank. If a species is left unanswered, its records simply come out blank, and a term with no answers at all produces no column — nothing is invented.
- **In the exported sheet both terms sit right after the taxon columns.** They are Occurrence terms, which would place them at the very front of the file, far from the species they describe. The export now relocates them to the end of the Taxon block, just after `vernacularName`, so opening the file shows the scientific name and immediately whether that species is native or introduced. Display order only: the Darwin Core class is unchanged, so the mapping grid still groups both cards under Occurrence and `meta.xml` still declares the real term URIs. The Preview follows the same order, since it is a preview of the export.
- **A card filled by the assistant now looks filled.** With no column selected it used to keep the grey MANUAL badge and an example line reading "column mapped, no values" — which was false, no column was mapped. Such a card now counts as mapped, carries its own ASSISTANT badge, and states what actually happened ("filled by the assistant: 7 species", plus "3 species still have no degree set" when the pair is incomplete).
- **The assistant button sits on the `establishmentMeans` card only.** One modal fills both terms, so the button belongs to the lead term; the `degreeOfEstablishment` card keeps its own column selector (your data still wins there) plus a note pointing at the card that owns the assistant.
- **A missing `degreeOfEstablishment` is reported, never blocking.** When a species gets an `establishmentMeans` but no degree, the assistant shows a running count while you work and warns again on save. The export proceeds normally: Darwin Core does not require `degreeOfEstablishment`, and a taxon whose degree is genuinely unknown must still be publishable. A degree saved without a means is dropped, since it says nothing on its own.
- **Name validation now flags invasive alien species.** Records whose validated name is on the Brazilian invasive alien species list (Instituto Hórus, 2023 — 483 taxa, animals and plants) get an "Invasive alien" badge in the results table, and a summary line above the table reports how many records are affected. The list is bundled with the app (`inst/extdata/invasive_species.rds`, regenerated by `data-raw/generate_invasive_species.R`), so the check is local, offline and instant. Matching uses exactly the same name normalization as the name validator, so authorship and qualifiers do not defeat it (`Felis catus (Linnaeus, 1758)` matches). Being on the list means the taxon is alien and invasive in Brazil, which is the basis for the `establishmentMeans` suggestion coming next; it deliberately says nothing about `degreeOfEstablishment`, which depends on the record (a captive animal and a feral one are the same species). A missing or unreadable list degrades to "nothing flagged" with a warning rather than breaking validation. An "Invasive" filter pill joins the existing ones above the processed names, so you can narrow a long list down to just the invasive taxa; unlike the other pills it filters on the species list rather than on validation status, so it is unaffected by reviews.
- **The `dynamicProperties` card now shows the JSON it will generate.** The term's card offered the per-column key editor but never an example, so there was no way to tell from the mapping screen whether `{"key":"value"}` was being assembled — a column mapped to `dynamicProperties` looked inert. The card now shows the assembled object for the first row (e.g. `{"cor":"melanico"}`) underneath the key editor, updating as keys are edited. The JSON generation itself is unchanged; only its visibility is new.

### Changed
- **The tutorials' downloadable sample is now Saíra's full demonstration dataset.** The help-site sample linked from the tutorials index and the Upload step (PT/EN) was a hand-built 12-row spreadsheet (`ocorrencias-exemplo.csv`); it is replaced by `ocorrencias-demo.csv`, the 1,075-occurrence demonstration dataset (48 real Brazilian species from all six Brazilian biomes, threatened taxa included, with deliberate imperfections), so anyone following the tutorials works with the same dataset used in the app demonstration. The four download links and the sample descriptions are updated; the old "one swapped coordinate and a blank country" wording is corrected, since the demonstration dataset has no blank country (the country-from-coordinates fix still exists in the app, it is just not exercised by this sample). Known follow-up: the mapping (03) and export (07) walkthroughs still describe the single-date `data_coleta` sample, whereas the demonstration dataset uses month/year interval date columns (`mes_inicio`/`ano_inicio`/`mes_fim`/`ano_fim`); their prose, example table and screenshots are not yet re-shot for the new dataset.

### Fixed
- **Switching the interface language no longer wipes your mapping.** Changing between Portuguese and English re-read the uploaded file from disk and handed the rest of the app a brand new table, which every downstream screen correctly interpreted as "a new file was uploaded": the column mapping was cleared, both assistants (`basisOfRecord` and the establishment pair) lost their answers, and the preview caches were dropped. Nothing was wrong with the reset itself, it is the right response to a new upload. The defect was upstream: the upload step read the language setting while parsing the file, purely to decide which language the "file loaded" notification should be written in, and that made the whole read depend on the language. The language is now read once, at the moment the file is actually loaded, so the parsed table depends only on the file you chose. Nothing else changes: notifications and error messages still appear in the language that was active when you uploaded, which is what a one-off message should do. A regression test pins this by swapping the file's contents on disk and asserting a language switch does not pick them up.
- **The establishment assistant button no longer translates the Darwin Core term.** In Portuguese the button read "Assistente de estabelecimento", turning `establishmentMeans` into a Portuguese phrase, which breaks the rule the rest of the app follows: a Darwin Core term is an identifier and stays in English in both languages, exactly as the `basisOfRecord` assistant button already did ("Assistente basisOfRecord"). Both labels now name the real term: "Assistente establishmentMeans" and "establishmentMeans Assistant".
- **A mapping card now always says what it is doing with the column you picked.** The one-line example under a card's column selector (`e.g.: Panthera onca`) was rendered only when the sampled rows had values; when the mapped column was blank in the first 200 rows the card rendered nothing at all, which is exactly what an unmapped card looks like. Mapping a column and getting silence back read as "the app ignored my mapping". The card now states the empty case explicitly ("column mapped, no values in the first 200 rows"), so "mapped but blank" and "not mapped" are no longer visually identical. This is a label only: the exported cell stays empty, per the Darwin Core blank convention (`replace_na_with_blank()` is unchanged), and a test asserts the hint never reaches the data.
- **A freshly mapped card no longer waits a beat before showing its example.** The card's dynamic slot read the mapping state only from `rv$map_values`, which the input-sync observer fills one reactive flush after the client echoes a selection. The mapping grid already fell back to the raw input for exactly this reason; the slot now does the same, so the example line appears on the same flush the column is chosen. Most visible on terms added through "Add term", where the card is new and the state starts empty.

### Performance
- **Tables and validation screens now build their labels once per distinct value instead of once per row.** Several screens called the translation function inside a per-record loop: the coordinate issues table translated its badge (and re-read the language setting) once for every row, the sensitive-species export wrote its Chapman release statement once for every masked row, and the name validation report normalized its status once for every row. The number of distinct values in all three cases is tiny and does not grow with the spreadsheet: nine coordinate diagnostics, five sensitivity tiers, five validation statuses. Each is now resolved once per distinct value and expanded back over the rows by index, the same approach the coordinate map layers already used. Measured over 50,000 rows: coordinate badges 1.92s to 0.005s, sensitivity statements 1.44s to 0.002s, status normalization 0.65s to 0.002s.
- **The translation lookup itself got 5.5x faster.** Every call to `tr()` scanned the environment chain and then the package namespace for a legacy dictionary object that no longer exists (the dictionary moved to `inst/extdata/i18n.json`), and then built a fresh 869-element vector of key names just to test whether the key was present. It now reads the already-warmed cache first and looks the key up directly. Both warning paths and the fallback to English are unchanged. Measured: 1.59s to 0.29s for 50,000 lookups. Since `tr()` is called from roughly 700 places, this compounds into every screen.
- **Counting records per sensitive species no longer rescans the whole column once per species.** The sensitive-coordinates screen ran one full-column comparison for each detected species, so the work grew with species times rows. It now makes a single tabulation pass over the georeferenced rows. This also fixes a latent crash: a row with coordinates but a blank scientific name could turn a count into `NA` and break the screen. Measured: 0.17s to 0.04s for 40 species over 50,000 rows.
- **The Darwin Core term list is built column by column.** Both builders re-indexed the term table four to five times per term inside a row loop, and the list is rebuilt on every language switch. Measured: 0.19s to 0.06s for 200 builds.
- Behavior is unchanged throughout: no screen, value or export differs. Regression budgets covering these paths were added to the opt-in performance suite (`RUN_PERF=true`), set to fail if a per-row loop returns.

## [0.9.4] - 2026-07-01

### Fixed
- **The exported EML now reflects the license chosen in the mapping.** The Darwin Core Archive's `eml.xml` `<intellectualRights>` was previously fixed to the CC0 public-domain waiver on every export, regardless of the license selected in the mapping — so a dataset licensed CC-BY 4.0 or CC-BY-NC 4.0 was published as public domain. The export now reads the chosen license from the mapped `license` value (the license card constant or a mapped column) and writes the matching statement in the GBIF/IPT format (a `<para>` with a `<ulink url><citetitle>` naming the license), for all three GBIF-accepted licenses (CC0 1.0, CC-BY 4.0, CC-BY-NC 4.0), recognizing both the short token and the full Creative Commons URL. CC0 is used only when no license is set; any unrecognized value is cited verbatim so the EML never silently claims CC0. Covered by tests over the token, URL, default and unknown paths.
- **`fundingAttribution` is now declared under its Audiovisual Core namespace in `meta.xml`.** The term `fundingAttribution` belongs to Audiovisual Core (`http://rs.tdwg.org/ac/terms/fundingAttribution`), but the archive descriptor emitted a `dwc:` URI for it like every other term. `dwc_term_uri()` now maps it (and any future `ac:` term) to the `ac:` namespace, mirroring the existing Dublin Core handling. Only affects archives that map a column to `fundingAttribution`.
- **Adding a term via the "Add term" modal now inserts it into the mapping and scrolls to its card.** Selecting a term in the "Add term" modal and confirming previously had no effect — no card showed up in the mapping grid. The cause was the recent grid-performance change: the grid (`output$mapping_ui`) reads the active term list (`dwc_all()`, which carries the session's extra terms) inside an `isolate()`, with only four reactive triggers (new upload, language switch, the show-only-mapped toggle, and the scientificName lock). Adding a term updates `rv$extra_terms` but matched none of those triggers, so the grid never re-rendered and the new card never appeared. The grid now takes an explicit dependency on the active term set, so adding or removing a term rebuilds it while per-column selections still update only their own card. After a term is added the app smooth-scrolls to the new card so it is immediately visible (the scroll now waits on a time budget rather than a fixed frame count, since rebuilding the ~50-input grid can take a moment). A regression test asserts the added term's card is present in the rendered grid.
- **Importing a mapping template now shows the cards for non-default terms it maps.** When a `mapping_guide.txt` referenced a Darwin Core term outside the default set (e.g. a `geodeticDatum` mapping carried over from another dataset), the import faithfully restored the value into the term's state but never registered the term as a session extra — so no card rendered and the restored mapping was invisible. The import now registers any catalog term outside the default set (via the same `detect_extra_dwc_terms()` path used for uploaded columns), so every term the guide maps gets a visible card. Covered by an integration test that imports a guide mapping `geodeticDatum` and asserts the card renders.
- **The Preview table now keeps its Darwin Core header frozen while you scroll the rows.** Previously the Preview tab scrolled the whole page vertically (the table itself only scrolled horizontally), so scrolling down to see more rows carried the header — the DwC term names — off the top of the screen, and you lost track of which column was which. The table now owns its vertical scroll (DataTables `scrollY`) and the Preview tab no longer page-scrolls, so the header row stays pinned and only the table body scrolls. The reserved height (`calc(100vh - 20rem)`) accounts for the navbar, title and table controls; horizontal scroll for wide datasets is unchanged.
...
- **45 new Darwin Core terms and 9 new classes from the current TDWG vocabulary are now in the mapping catalog and Wiki.** The Darwin Core catalog was resynced against TDWG's published recommended term list (217 → 262 terms), adding the DwC-DP classes **Agent, Assertion, BibliographicResource, MolecularProtocol, NucleotideAnalysis, NucleotideSequence, OrganismInteraction, Protocol and Provenance** and their terms (e.g. `agentID`, `assertionType`, `referenceID`, `protocolID`, `organismInteractionType`, `sequence`). Every new term ships with a curated PT-BR definition, so the nine classes appear as filter groups in the Wiki and as optgroups in the "Add term" modal, fully bilingual. No terms were removed upstream. The catalog is regenerated from live upstream by `data-raw/build_dwc_full_catalog.R`.

### Changed
- **Eight Darwin Core terms are now grouped under the class TDWG assigns them.** `catalogNumber`, `otherCatalogNumbers`, `discipline` and `typifiedName` are now **MaterialEntity**; `projectTitle`, `projectID`, `fundingAttribution` and `fundingAttributionID` are now **Provenance**. This corrects the by-class grouping in the mapping grid and the Wiki. In the exported spreadsheet, `catalogNumber`/`otherCatalogNumbers` now lead the MaterialEntity column block instead of sitting in the Occurrence block; the values written are unchanged.

## [0.9.3] - 2026-06-25

### Changed
- **The Help tab is now a resource hub instead of a workflow recap.** The five-step workflow stepper (which duplicated the website tutorials) was removed and replaced with a **Resources** section that puts the external references in evidence: a direct link to the full **tutorials** on the app website (language-aware PT/EN), the **useful links** (Darwin Core / TDWG, SiBBr, GBIF) promoted out of the sidebar, a direct **GitHub issues** link, and the three GBIF **best-practice PDFs** behind Saíra's generalization and georeferencing (Chapman 2020; Chapman & Wieczorek 2020; Zermoglio et al. 2020). A condensed **FAQ** is now available in-app behind a click-to-expand toggle (the six most common questions, each collapsible) with a link to the full FAQ on the site, and the Help tab now scrolls correctly when that content overflows the viewport. The **Built with** card now lists every runtime dependency, each chip linking to the package's page. All text stays bilingual (PT/EN).
...
- **`eventDate` and `dateIdentified` are normalized to ISO 8601 in the mapping preview, and unpadded dates now convert.** The mapping card preview showed the raw user value (e.g. `01/12/2026`) while only the export converted it to ISO 8601, so the preview disagreed with the file. `build_term_value()` now applies the same `parse_dates_to_iso()` normalization the export uses, so the preview matches what is written. Two parsing gaps are also fixed in `parse_dates_to_iso()` (shared by preview and export): (1) dates without zero-padding (`2/9/2021`, `5.3.2016`) previously did not match and were kept raw — they now parse like their padded forms; and (2) day-first (`DD/MM`) vs month-first (`MM/DD`) is now decided per column by voting over the whole column — any value whose first component is `> 12` proves day-first, any second component `> 12` proves month-first — so a column like `15/03/2020, 9/6/2017` is read consistently from the data itself. A fully ambiguous column (every component `<= 12`, no disambiguating row) falls back to month-first (`MM/DD`), the safer guess for English-published datasets. Multi-column `eventDate` intervals (`YYYY-MM/YYYY-MM`), year-only values (`2026`) and unparseable text keep their raw value unchanged.

## [0.9.2] - 2026-06-24

### Added
- **The running version is shown in the navbar, and the app warns when your R session is stale.** A clickable version badge (e.g. `v0.9.2`, styled like the website's navbar pill) now sits in the navbar next to the language selector so you can confirm at a glance which version you are running; it links to the releases page on the help site (language-aware, opens in a new tab). The badge shows the version **loaded in the session**, not the one on disk, so it stays truthful when R was not restarted after an update. If you update Saíra (via `pak`/reinstall) **without restarting R**, the session keeps executing the previously loaded namespace while the newer version sits on disk, so the app appears unchanged — this was the root cause of a reported "mapping cache stuck on the old version". The app now detects this by comparing the loaded namespace version against the installed `DESCRIPTION` (`saira_session_is_stale()`), and raises a sticky in-app warning telling you to restart R plus a matching console `warning()`. Dev sessions under `pkgload::load_all()` are never flagged (both versions resolve to the repo). The FAQ gains a "I updated but it looks like the old version" card in PT and EN.

### Fixed
- **The conservation-status summary line no longer crowds the validation stats.** The "MMA/IUCN status will be added…" band that sits between the valid/invalid/unresolved counts and the names report now has a clear top separator and more vertical breathing room, so at narrower report widths it no longer appears to touch the statistics above it.
- **Flora BR / Fauna BR downloads work again (they were stuck on "update failed").** The Brazilian providers never finished downloading: validating a dataset with plant species left Flora BR in a permanent `update_failed` state and an empty cache. Two upstream changes in the `florabr`/`faunabr` packages broke the bootstrap: (1) `florabr::get_florabr()` now writes its artifact as `CompleteBrazilianFlora.rds` (it used to be `Flora_e_Funga_do_Brasil.rds`), so our post-download artifact check failed and aborted every download; and (2) `load_florabr()`/`load_faunabr()` no longer resolve `data_version = "latest"` to the downloaded version folder (e.g. `393.427/`), so loading right after a `"latest"` download failed with "cannot open the connection". The download path now matches the current artifact name and resolves `"latest"` to the actual downloaded version before loading. The `"latest"` resolution applies to both providers; Flora BR is fully restored in both PT and EN.
- **Fauna BR downloads work again.** Beyond the `"latest"` resolution above, the Fauna BR bootstrap also hit an upstream crash: the Catálogo Taxonômico da Fauna do Brasil published its most recent data versions with an empty `locality` column, which made `faunabr`'s internal `merge_data()` evaluate `if (!is.na(group$locality))` on a length->1 vector and abort with "the condition has length > 1". This is not Saíra code and cannot be worked around from our wrapper. The package maintainer fixed it in the development build, but CRAN `faunabr` 1.0.0 and 1.0.1 are both still affected, so `DESCRIPTION` now pins `faunabr` to the fixed commit (see Changed). Verified end to end: download + load of 192,382 taxa.
- **Large provider downloads no longer time out on slow connections.** The IPT archives are large and R's default 60s download timeout could truncate the zip, surfacing as an "extracting from zip" failure and a stuck `update_failed` card. `.brprovider_download_data_impl()` now raises the timeout to at least 600s for the duration of the download (restored on exit), for both Flora BR and Fauna BR.

### Changed
- **`faunabr` is now sourced from the maintainer's fixed development build.** `DESCRIPTION` gains `Remotes: github::wevertonbio/faunabr@e3d4331` (recorded in `renv.lock`) because the empty-`locality` crash above is unfixed in CRAN `faunabr` 1.0.0/1.0.1. `pak::pak("rogerio-onza/saira")` resolves the pinned commit automatically when installing Saíra. Revert to the CRAN package and drop the `Remotes` entry once a fixed `faunabr` reaches CRAN.
- **Help site: "Flora BR / Fauna BR won't download? Update Saíra" notice (docs only).** The name-validation tutorial (PT and EN) gains a callout explaining that the Brazilian source files change over time and that a stuck "update failed" card means the app needs reinstalling with `pak` (`pak::pak("rogerio-onza/saira")`), which pulls the correct database packages automatically.
...
### Added
- **Conservation status (global IUCN + Brazilian MMA) is now auto-added to `dynamicProperties` on export.** Every exported record for an assessed taxon carries its threat status as clean, flat JSON keys, keyed to the providers chosen in the Name-validation tab and **additive** — a taxon can receive both at once. (1) When a Brazilian provider (Flora BR / Fauna BR) is selected, taxa on the bundled national list get `mmaThreatStatus` (e.g. `"CR"`) plus `mmaSource` citing the portaria that listed them (e.g. `"Portaria 1.704/2026"`); this is fully local. (2) When **GBIF** is selected — it is pre-selected by default — taxa get `iucnRedListCategory` (the IUCN code, e.g. `"NT"`) read from GBIF's keyless `…/species/{usageKey}/iucnRedListCategory` endpoint, reusing the `taxonID` taxadb already resolved (falling back to the keyless `/species/match` for rows a Brazilian provider matched). A taxon GBIF does not assess simply keeps its MMA keys, and the entries merge into any `dynamicProperties` you already mapped, e.g. `{"mmaThreatStatus":"CR","mmaSource":"Portaria 1.704/2026","iucnRedListCategory":"NT"}`. The GBIF call is the app's first outbound request and is strictly **optional and non-blocking**: `httr2` is a Suggests-only dependency, results are memoized per session, and a missing package, an offline machine or any API error degrades to no IUCN key without ever breaking or stalling the export. A summary line above the validation report shows how many records will receive each status. The bundled `sensitive_species.rds` gains a `source` column carrying the portaria provenance (flora 148/2022, terrestrial fauna 1.704/2026, aquatic fauna 1.667/2026).
- **Fixed (constant) values for more dataset-level Darwin Core terms.** Extending the existing `datasetName`/`license`/`language` behaviour (ADR-005), seven more dataset-level terms can now be stamped with a single value applied to **every row** instead of mapping a column: `rightsHolder`, `institutionCode`, `collectionCode`, `country`, `references`, `bibliographicCitation`, and `geodeticDatum`. Each eligible mapping card gains an opt-in **"use a fixed value"** checkbox (with a one-line hint that it ignores the column mapping) that reveals a text field plus a tinted strip making clear the value fills every row at export; the card stays unchanged until you opt in. Mapping a column and using a fixed value are mutually exclusive — an **OR** divider separates them, and ticking the box hides the column selector so a constant can never silently overwrite a column you had mapped. The card's mapped outline and status badge (**EDITADO**) update as soon as you finish typing, without re-rendering the grid. Terms not shown by default (`references`, `bibliographicCitation`, `geodeticDatum`) get the option as soon as they are added via the existing **Add term** button. `countryCode` (derived during coordinate validation) and `basisOfRecord` (which has its own per-value assistant) are intentionally excluded. A user-set `geodeticDatum` is preserved by the export auto-fill, which only touches blank rows.
- **Coordinate generalization is now fully Chapman 2020-compliant: it never makes coordinates look *more* precise than they already are, and never drops the protection metadata.** Generalizing rounds to a coarser grid (e.g. `extreme` = 1°, `high` = 0.1°), but per Chapman §4.1/§4.3 generalization must *reduce* precision, never invent it — rounding a 2-decimal `-81.41` to the 0.001° grid would falsely imply ~110 m. The masking pipeline now detects each record's existing precision (decimal places of `decimalLatitude`/`decimalLongitude`, plus any `coordinatePrecision`) and **clamps the grid so it is never finer than that** (`effective grid = max(chosen tier, existing precision)`). Crucially the record is **still masked and documented** — `informationWithheld`, `dataGeneralizations`, `coordinateUncertaintyInMeters`, `coordinatePrecision`, and locality scrubbing all apply (Chapman §5.1) — only the grid is widened; nothing is silently skipped. `dataGeneralizations` now also records the **precision originally stored** (precision provided + precision stored, per Chapman §4.3). The Generalization tab shows an informational banner counting records published at their real precision (clamped, or already generalized upstream per ADR-095), making the previously-silent behaviour visible. Datasets already at low precision (e.g. coordinates published from a paper at two decimals) get protected without ever being re-rounded into false precision.
