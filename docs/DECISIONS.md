...
## ADR-108: Card de mapeamento sempre declara o que fez com a coluna (estado vazio explicito + previa de dynamicProperties)

**Contexto.** A linha de exemplo do card (`.field-card-sample`, ADR do painel de mapeamento) so era renderizada quando a amostra tinha valores: `build_field_sample()` devolvia `NULL` para `character(0)`. Consequencia pratica reportada pelo dono: mapear uma coluna cujas primeiras 200 linhas estao em branco produz um card **identico ao de um termo nao mapeado**, o que se le como "o app ignorou meu mapeamento". Some-se a isso que (a) o slot `carddyn_<term>` lia so `rv$map_values[[term]]`, preenchido pelo observer de sincronia um flush **depois** do eco do cliente — enquanto a grade ja tinha fallback para `input[[map_<term>]]` exatamente por isso — e (b) `dynamicProperties` era excluido da previa por decisao anterior, entao nao havia como saber, da tela de mapeamento, se o `{"chave":"valor"}` estava sendo montado.

**Decisao.**
1. **Estado vazio explicito.** `build_field_sample()` deixa de devolver `NULL`. A funcao so e chamada para termo **ja mapeado** (ambos os call sites guardam com `has_selected_value`), entao amostra vazia significa inequivocamente "mapeado, mas as linhas amostradas estao em branco" e passa a renderizar `.field-card-sample.field-card-sample-empty` com a chave `mapping_card_sample_empty`. Sem parametro de configuracao: nao existe caminho em que a funcao seja chamada para termo nao mapeado.
2. **Rotulo de UI, nunca dado.** O `div` nao tem caminho para `build_processed_mapping_df()`. Coluna mapeada e vazia continua exportando `""` via `replace_na_with_blank()`, a convencao de branco do DwC. Teste explicito de nao-vazamento (`preview_data_r()` da coluna == `""`, sem o texto da dica).
3. **Paridade de leitura no slot.** `carddyn_<term>` passa a usar o mesmo fallback da grade (`rv$map_values[[term]]` e, se `NULL`, `input[[map_<term>]]`), entao a linha de exemplo aparece no mesmo flush da escolha da coluna. Mais visivel em termos vindos do "Adicionar termo", onde o estado comeca vazio.
4. **`dynamicProperties` ganha previa.** O slot passa a emitir o bloco de chaves **e** `build_field_sample()` com o JSON montado da primeira linha (ex.: `{"cor":"melanico"}`). O termo ja estava fora do memo `preview_cache` (`cacheable <- !(term %in% c("basisOfRecord","dynamicProperties"))`), entao a previa acompanha a edicao das chaves em tempo real. Revoga a exclusao de `dynamicProperties` do item 4 do ADR do painel de mapeamento; `basisOfRecord` **continua** excluido (o card mostra o botao do assistente, nao uma amostra).

**Rejeitado (hipotese refutada na implementacao).** Limpar o gate `rendered_map_inputs` a cada re-render da grade, sob a hipotese de que recriar os ~50 `selectInput` fazia o cliente devolver `NULL` e o observer de sincronia apagar `rv$map_values`. Verificado no fonte do Shiny instalado (`www/shared/shiny.js`, `unbindInputs()`): o unbind e **so local** — remove a classe, desregistra o binding, dispara `shiny:unbound`, e **nao envia valor ao servidor**. Nao existe rajada de `NULL`. Alem de desnecessario, seria regressivo: apos um re-render o Shiny nao reenvia valores inalterados, entao o gate ficaria vazio e um "limpar o campo" legitimo (que ai sim manda `NULL`) passaria a ser ignorado. O gate do ADR-104 fica como esta.

**Consequencias.** i18n: `mapping_card_sample_empty` (pt/en). CSS: `.field-card-sample-empty` (italico + opacidade, herdando `.field-card-sample`; flat, sem `!important`, bundle segue em 11). Testes: contrato novo de `build_field_sample` em `test-mod-mapping-cards.R` (o teste antigo afirmava `expect_null` e foi substituido); em `test-mod-mapping-server.R`, termo adicionado + mapeado chega na preview e mostra a amostra, coluna em branco mostra a dica e exporta `""`, e o card de `dynamicProperties` mostra o JSON. Sem mudanca no contrato de lista nomeada do `mod_mapping_server` (ADR-054).

## ADR-109: Lista de especies exoticas invasoras embarcada (Instituto Horus 2023), flag na validacao de nomes

**Contexto.** O app ja identifica taxons **ameacados** (lista MMA, ADR-092/ADR-107) mas nao dizia nada sobre taxons **exoticos invasores** — o polo oposto do mesmo eixo e um dado que muda a interpretacao do registro. O dono forneceu a "Lista de especies exoticas invasoras do Brasil" (Instituto Horus, 2023). Alem do valor proprio na validacao, essa lista e o insumo da sugestao automatica de `dwc:establishmentMeans` (ADR seguinte): estar na lista sustenta `introduced`.

**Decisao.**
1. **Mesmo padrao de lista embarcada do MMA, com a fonte fora do repo.** `data-raw/generate_invasive_species.R` -> `inst/extdata/invasive_species.rds` (483 taxa: 271 Animalia, 207 Plantae, 4 Protista, 1 Monera), consumido por `R/utils_invasive.R` espelhando `R/utils_sensitive.R`: `create_rds_cache("invasive_species")` (ADR-014), `resolve_*_path()` com tres candidatos (instalado / raiz do projeto / `../..` do testthat), `*_empty()` com o schema exato, e degradacao graciosa — arquivo ausente ou invalido vira `warning()` + frame de zero linhas (nada e sinalizado), nunca erro. O caso "nao encontrado" **nao** e cacheado, entao regenerar o RDS e visto sem `force = TRUE`.
2. **Chave de casamento em lockstep com o validador.** `build_invasive_match_keys()` aplica exatamente os dois passos que o gerador aplica: `normalize_scientific_name(remove_authors = TRUE, ignore_qualifiers = TRUE)` e depois `normalize_for_matching()`. Consequencia deliberada: o comportamento herdado do normalizador vale tambem aqui — `Sus Scrofa` (segunda palavra capitalizada) e lido como autoria e vira `Sus`, entao **nao** casa. A lista casa igual ao que a validacao resolve, nem mais frouxo nem mais estrito; ha teste fixando isso.
3. **Fonte do nome canonico.** A planilha tem **duas linhas de cabecalho** (a segunda e rotulos: "Nome Cientifico", "Autor", ...), descartada no parse. `Supplied Name` traz o binomio limpo e `scientificName` a forma com autoria, **em branco em 27 linhas** — o nome canonico cai de um para o outro. `Motivo da introducao no Brasil` e preservado como `introduction_reason` para um futuro mapeamento de `dwc:pathway`; **nao** e `degreeOfEstablishment`.
4. **UI: badge na celula, sem coluna visivel nova.** Segue a escolha do ADR-107 item 6 (nao mexer na largura do DT): `.is_invasive` entra como coluna **oculta de indice 6** em `table_df`, e o render da coluna 0 (`scientific_name_js`) le `row[6]` e empilha `<div class="vn-cell-invasive"><span class="vn-status-badge badge-error">` abaixo da pill de sensivel. Escape client-side pelo idioma `$('<div/>').text(x).html()` ja usado no arquivo. Resolucao por `flag_invasive_species()`, que dedupe internamente (`unique` + `match`) — mesma economia do `sensitive_resolve` no mesmo render.
5. **Contagem agregada reusa `conservation_status_summary_ui()`.** Uma linha nova ("N registro(s) de especie exotica invasora"). Diferenca importante: as linhas MMA/IUCN sao condicionadas ao provedor selecionado, a linha de invasoras **nao** — a lista e local e nao depende de provedor. Por isso o early-return da funcao deixou de exigir `include_mma || include_iucn` e passou a exigir so nomes na entrada.
6. **Pill de filtro num eixo novo.** A barra de pills sobre os nomes processados ganha "Invasoras". As demais pills filtram por `validation_status`; essa filtra pela lista de especies (`flag_invasive_species(query_name)`). Consequencia deliberada: ela **nao** interage com `reviewed_keys`/`exiting_keys` — revisar um nome nao muda o fato de o taxon estar na lista, ao contrario dos filtros de problema, que suprimem linhas ja revisadas. `.vn_stream_filter_values` ganha `"invasive"`, e `stream_filter_counts()`/`filter_stream_df()` ganham o ramo correspondente. Cor propria (`.pill-invasive`, `#8c4a00`, contraste 6.79:1 com branco) em vez de reusar o vermelho de `pill-error`, que ali significa "nao encontrado": estar na lista nao e um erro.
7. **Nada e escrito no dado neste ADR.** A flag e display. Escrever `establishmentMeans`/`degreeOfEstablishment` e assunto do ADR seguinte, e passa pela decisao explicita do usuario no assistente.

**Divergencia deliberada do padrao MMA.** A planilha-fonte do Horus **nao** e versionada (`.gitignore`), ao contrario de `data-raw/redlist_brasil_mma.md`. Decisao do dono: so o RDS gerado interessa ao repo. Custo aceito: regerar exige obter a lista de novo e salva-la em `data-raw/invasive_species_horus_2023.csv`. A proveniencia sobrevive na coluna `source` de cada linha do RDS e no cabecalho do gerador.

**Consequencias.** `DESCRIPTION` `Collate` ganha `utils_invasive.R`; `.onLoad()` pre-aquece o cache junto com os demais. i18n: `validate_names_status_badge_invasive`, `validate_names_conservation_summary_invasive` (pt/en). CSS: `.vn-cell-invasive` so com espacamento — o badge reusa `.vn-status-badge` + `.badge-error` (flat, sem `!important`; bundle segue em 11). Testes: `test-utils-invasive.R` (casamento com/sem autoria, branco/NA/vazio, lista ausente degrada, schema do `*_empty()`, assercoes sobre o artefato distribuido, paridade da chave com o gerador); `test-mod-validate-names-server.R` estende a assercao de colunas ocultas (4/5 -> 4/5/6, `row[6]`, `vn-cell-invasive`) e ganha um teste de que `Sus scrofa` e sinalizado e `Panthera onca` nao. Reusa ADR-014 (cache), ADR-092 (padrao de lista embarcada), ADR-107 (sem coluna nova no DT).

## ADR-110: Assistente por especie para establishmentMeans e degreeOfEstablishment, com sugestao vinda da lista de invasoras

**Contexto.** Os dois termos tem vocabulario controlado TDWG (7 e 11 valores) e sao acoplados na pratica: quem declara `establishmentMeans` deveria declarar o grau. Preencher isso a mao numa planilha com milhares de linhas e inviavel, e nenhum dos dois estava no conjunto base do mapeamento (so no catalogo completo, classe `Occurrence`). O ADR-109 acabou de embarcar a lista de exoticas invasoras, que e exatamente o insumo da sugestao automatica.

**Decisao.**
1. **Chaveado na ESPECIE, nao no valor de uma coluna.** Diferenca central em relacao ao assistente de `basisOfRecord` (ADR-016/017/018), que traduz os valores unicos de uma coluna mapeada. Aqui as entradas sao as especies unicas da coluna mapeada em `scientificName` (`extract_species_entries()`), com contagem de registros por taxon e ordenacao por frequencia decrescente — as especies que cobrem mais dado aparecem primeiro. Uma resposta vale para todos os registros daquele taxon: e isso que torna a planilha grande tratavel. Precondicao: sem `scientificName` mapeado o assistente nao abre (notifica).
2. **Sugestao so onde ha base factual.** `auto_suggest_establishment_means()` pre-preenche `introduced` para taxons na lista do Instituto Horus (ADR-109) e **nada** para os demais — chutar `native` para um taxon nao listado afirmaria algo que o app nao sabe. `degreeOfEstablishment` **nunca** e sugerido: pela definicao TDWG (`invasive` = "populacao autossustentavel ... sobrevivendo e se reproduzindo a distancia significativa do ponto de introducao") o grau e propriedade do REGISTRO, nao da especie — um `Sus scrofa` em cativeiro nao e `invasive`. Escolha do usuario, sempre.
3. **Dado do usuario vence; o assistente so completa.** Em `build_processed_mapping_df()` os dois termos ganham um ramo antes da checagem de coluna mapeada: calcula o valor da coluna (se houver) e preenche os brancos com a resposta por especie via `fill_missing_character_values()` (o mesmo idioma ja usado para `genus`/`specificEpithet`). Isso e o oposto da precedencia de `constant_values`, que sobrescreve — aqui a planilha da pessoa e autoritativa. `build_establishment_term_value()` e o ponto unico dessa mesclagem, compartilhado com a linha de amostra do card, para os dois nunca divergirem.
4. **Vetor de especies resolvido do dado bruto, nao de `df_final`.** Assim a mesclagem nao depende de onde `scientificName` cai na ordem do laco de termos.
5. **Par recomendado, nao obrigatorio (decisao do dono).** `establishment_pairs_missing_degree()` alimenta uma contagem viva no modal e um aviso ao salvar; o export segue normal. O DwC nao torna `degreeOfEstablishment` obrigatorio, e um taxon de grau genuinamente desconhecido precisa continuar publicavel. Grau salvo sem `establishmentMeans` e descartado (nao diz nada sozinho).
6. **Progresso vivo sem cascata reativa.** O bloco de progresso/aviso le diretamente os inputs da pagina visivel (no maximo 20), entao os numeros se movem enquanto a pessoa escolhe, sem nenhum observer escrevendo `rv`. Preserva a propriedade do ADR-017 (sincronia por snapshot em Prev/Next/Salvar) e ainda assim da retorno imediato.
7. **Sem reset ao trocar a coluna de `scientificName`.** Diferenca deliberada em relacao ao `basisOfRecord`, cujas chaves sao valores daquela coluna especifica. Aqui as chaves sao nomes de especie: se a coluna muda mas as especies sao as mesmas, as respostas continuam validas; chaves obsoletas simplesmente nao casam e devolvem "". Resetar seria destrutivo sem ganho. O reset acontece em novo upload e no Reset do mapeamento.
8. **Promovidos ao conjunto base.** `data-raw/build_dwc_terms.R` passa a incluir os dois (64 -> 66 termos, classe `Occurrence`, definicao PT ja no catalogo). Ter de cacar o termo em "Adicionar termo" antes de ver o assistente anularia o ganho. O botao do assistente aparece **sempre** no card (diferente do de `basisOfRecord`, que so aparece com coluna escolhida): o assistente e util justamente quando a coluna do termo nao existe, que e o caso comum.
9. **Botao do assistente so no card do termo condutor.** Um unico modal preenche os dois termos, entao renderizar o botao nos dois cards colocava **dois inputs com o mesmo id** (`open_establishment_assistant`) na pagina — comportamento indefinido no Shiny, e invisivel para o `testServer`, que seta o input direto sem passar pelo DOM. O botao fica so em `establishmentMeans` (grau sem meio nao significa nada, e o salvamento ja descarta esse caso); o card de `degreeOfEstablishment` recebe uma nota apontando para o card que tem o assistente. A nota e uma ancora HTML simples (`href="#...fieldcard_establishmentMeans"`), sem input Shiny, entao nao reintroduz um segundo id. Os **cards** continuam os dois: o de grau e o caminho de coluna mapeada, sem o qual a precedencia do item 3 nao existe. Teste de regressao fixa que o id aparece num slot e nao no outro.
10. **Card reflete o estado do assistente.** Depois de responder o assistente sem mapear coluna, o card continuava lendo como intocado: seletor em `--`, badge cinza MANUAL e uma linha de amostra dizendo "coluna mapeada, sem valores nas primeiras 200 linhas" — **falsa**, nenhuma coluna estava mapeada. Corrigido em tres frentes: `is_field_mapped` passa a considerar respostas do assistente (borda verde, como ja faz para `occurrenceID`/`license`/valor fixo); um status **ASSISTENTE** entra no vocabulario de badges existente (AUTO/SUGERIDO/AMBIGUO/EDITADO/ALIAS/TEMPLATE/MANUAL); e o card ganha uma nota de estado ("preenchido pelo assistente: N especie(s)" e, quando ha pendencia, "N especie(s) ainda sem grau definido") no lugar da frase generica, que so aparece agora quando ha valor de fato. **Alternativa rejeitada:** um checkbox tornando o campo bloqueante como `occurrenceID`. A comparacao nao se sustenta — `occurrenceID` e sempre derivado, sem escolha; aqui os dois caminhos sao legitimos e **deliberadamente nao exclusivos** (coluna parcial + assistente completando os brancos e caso real). O checkbox mataria essa capacidade e custaria um clique a mais para resolver um problema que era de exibicao.
11. **Ordem no dado exportado: depois do bloco Taxon.** Os dois sao termos `Occurrence`, o que os jogaria para o inicio da planilha, longe da especie que descrevem. `dwc_canonical_class_overrides()` os realoca para o fim do bloco `Taxon`, logo apos `vernacularName`: quem abre o arquivo ve o nome cientifico e, ao lado, se a especie e nativa ou introduzida. E ordem de exibicao apenas — a classe no catalogo nao muda, entao a grade de mapeamento continua agrupando os dois cards sob `Occurrence` e o `meta.xml` continua declarando as URIs reais dos termos. Vale para Preview e export juntos, que usam a mesma funcao de ordenacao (a Preview e uma previa do export; divergir seria mentir).
12. **Acessores genericos.** `dwc_vocab_terms()` / `dwc_vocab_choices()` / `is_valid_dwc_vocab_term()` recebem o catalogo em vez de replicar os acessores de `basisOfRecord` mais duas vezes.

**Consequencias.** `DESCRIPTION` `Collate` ganha `mod_mapping_establishment_assistant.R`. `inst/extdata/dwc_terms.rds` regenerado (66 termos). Os dois termos entram na lista de nao-cacheaveis de `processed_preview_for_term` (o valor depende de `rv$establishment_map`, nao so da coluna). i18n: 18 chaves `est_assistant_*` + 2 `a11y_est_*` (pt/en). CSS: o modal carrega `.bor-assistant-modal` **e** `.est-assistant-modal`, entao reusa todo o bloco existente; so a coluna de especie, o selo de invasora e a linha de aviso sao novos (flat, sem `!important`; bundle segue em 11). Testes: helpers puros e precedencia em `test-utils-mapping.R`; abertura/sugestao/salvamento/preview, par incompleto, ausencia de `scientificName` e reset em novo upload em `test-mod-mapping-server.R`. Reusa ADR-017 (snapshot), ADR-018 (vetorizacao), ADR-109 (lista de invasoras).

**Segunda armadilha registrada.** A grade so re-renderiza em mudanca estrutural (ADR-098), entao escrever o estado do card dentro do `renderUI` da grade nao bastava: apos salvar o assistente os dois cards ficavam com o badge e a borda antigos. O caminho correto e `push_card_state()`, o mesmo refresh cirurgico que uma selecao de coluna usa, disparado por um `observeEvent(rv$establishment_map)`. Como ele empurra via `session$sendCustomMessage`, o estado **nao** aparece no HTML que o `testServer` expoe — por isso o teste afirma sobre `apply_establishment_card_state()`, o helper compartilhado pelos dois caminhos, e nao sobre o HTML da grade.

**Armadilha registrada.** `[[chave]]` em vetor nomeado **erra** ("subscript out of bounds") quando a chave nao existe, ao contrario de lista, que devolve `NULL`. Custou dois bugs na implementacao (sincronia da pagina e leitura da linha do modal). O assistente de `basisOfRecord` ja guardava isso com `%in% names(...)`; aqui o mesmo cuidado vale, ou `[chave]` de colchete simples, que devolve `NA`.

## ADR-111: Lote de desempenho nos caminhos quentes de renderizacao -- tirar `tr()` e normalizacoes escalares de dentro dos lacos por linha

**Contexto.** Auditoria de desempenho pedida pelo dono, restrita a mudancas de risco baixo: nenhuma pode tocar contrato ja fixado por ADR (ADR-002 `max_rows`, ADR-020 `priority`, ADR-053 debounce de idioma, ADR-093 cap de streaming, ADR-098 `isolate()` da grade, ADR-102 `suspendWhenHidden`, ADR-104 gate `rendered_map_inputs`). A medicao mostrou um padrao unico repetido em varios modulos: **funcoes escalares chamadas uma vez por registro**, quando o numero de valores distintos e minusculo (5 tiers, 9 diagnosticos, 5 status) e independente do tamanho da planilha.

**Decisao.**
1. **`tr()` deixa de pagar duas varreduras de ambiente e um `names()` por chamada.** `resolve_i18n_dict()` le o cache quente (`i18n_cache`, ADR-014) antes dos `exists("i18n_dict", inherits = TRUE)` / `exists(envir = asNamespace("saira"))`, que sozinhos eram ~60% do custo e nunca encontram nada que o loader ja nao devolva (nenhum objeto `i18n_dict` existe no pacote desde a migracao para JSON, ADR de Onda 5). Os dois ramos legados continuam como caminho frio (cache vazio: carga parcial em teste, ou antes do `.onLoad()`), entao a sobrescrita por ambiente ainda resolve quando e a unica fonte. E `!key %in% names(dict)` vira `entry <- dict[[key]]; is.null(entry)`: `names()` materializava um vetor de 869 elementos a **cada** chamada. `dict` e lista, entao `[[` devolve `NULL` para chave ausente em vez de erro (ao contrario de vetor nomeado, armadilha registrada no ADR-110). Os dois `warning()` e o fallback para ingles ficam intactos. Medido: 1,586s -> 0,288s para 50k chamadas (**5,5x**), ganho que se multiplica em todo o resto, ja que `tr()` aparece ~700x em `R/`.
2. **Rotulos por linha viram mapa nomeado + expansao por indice.** Mesmo idioma que `build_coord_map_layers()` (`utils_coords.R`) ja usava: resolver sobre `unique()` e expandir com `map[vec]` ou `match()`. Aplicado em tres pontos: badges da tabela de coordenadas (`mod_validate_coords.R`, que ainda chamava `lang_r()` **por registro**, uma leitura reativa por linha), `sensitive_reason_statement()` (declaracao Chapman por linha mascarada) e `normalize_status_for_filter()` nos tres call sites de `mod_validate_names.R`, via um companheiro vetorizado `normalize_status_vec()` posto ao lado do escalar em `mod_validate_names_helpers.R`. Tambem `query_name`, cuja normalizacao passa a rodar sobre os nomes distintos. Medido em 50k linhas: badges 1,915s -> 0,005s, declaracao Chapman 1,443s -> 0,002s, status 0,647s -> 0,002s.
3. **Contagem de registros por especie deixa de ser quadratica.** `vapply(sens, function(s) sum(sci == s & has_coord), integer(1))` em `mod_sensitive_coords.R` varria a coluna inteira uma vez por especie sensivel. Vira um `table(sci[keep & has_coord])` indexado por nome. Efeito colateral corrigido: `table()` ignora os nomes NA/vazios que `keep` ja excluia, entao uma linha sem `scientificName` mas com coordenada nao produz mais `NA` na contagem (o `sum(NA & TRUE)` antigo devolvia `NA`, e `if (!any(ok))` com `NA` erra). 0,165s -> 0,038s para 40 especies x 50k linhas.
4. **Construtores da lista de termos DwC passam a ser column-wise.** `get_dwc_terms_list()` e `get_active_dwc_terms_list()` reindexavam o data frame quatro a cinco vezes por termo dentro de um `lapply(seq_len(nrow(...)))`. Extracao das colunas uma vez + `Map()`. O `required` sai para `dwc_required_flags()`, que preserva exatamente o contrato do `isTRUE()` por linha (TRUE so para logico nao-NA verdadeiro; coluna ausente ou nao-logica -> tudo FALSE). 0,19s -> 0,06s para 200 chamadas (**3x**). O numero parecia irrelevante numa primeira medicao (0,001s) por um **erro de harness**, registrado em `LESSONS.md`.
5. **O observer de sincronia do mapeamento MANTEM a dependencia de idioma.** Estava no plano remove-la (`all_term_names()` sob `isolate()`, guardando so `rv$extra_terms`) sob a leitura de que reexecutar a varredura dos 66 termos numa troca de idioma era desperdicio, ja que nenhum valor de mapeamento depende de idioma. **Errado, e reprovado em teste manual: trocar de idioma apagava o mapeamento inteiro.** `output$mapping_ui` tambem depende de `lang_r()`, entao a troca de idioma **destroi e recria os ~66 `map_<termo>`**. A dependencia fazia o observer re-rodar no mesmo flush dessa reconstrucao, reafirmando `rv$map_values` a partir dos inputs ainda validos antes da grade ser remontada a partir dele. Sem ela o observer so acorda quando o cliente re-vincula os inputs novos, le cada um como `NULL` com `rendered_map_inputs` ja `TRUE`, e cai no ramo "usuario limpou o campo" do gate da ADR-104. Ou seja: a leitura de `lang_r()` nao era custo acidental, era o acoplamento que mantem o observer em passo com a re-renderizacao que o proprio idioma dispara. Comentario no codigo registra isso no ponto exato. **Nenhum teste automatizado pega isso** — o `testServer` nao simula o unbind/rebind de inputs no DOM, entao a suite passava verde; o gate de teste manual foi o unico detector.

**Consequencias.** Nenhuma mudanca de comportamento observavel: os testes existentes dos cinco modulos passam sem edicao. Novas travas em `test-performance-regression.R` (opt-in por `RUN_PERF=true`) com orcamento posto bem acima do custo vetorizado e bem abaixo do custo por linha que substituiu, para falhar se um laco por linha voltar sem penalizar runner lento. Cobertura nova: paridade de `normalize_status_vec()` com o escalar (incluindo vazio, NA e fator), preservacao de ordem de `sensitive_reason_statement()` com tiers repetidos, contrato de `dwc_required_flags()`, e os dois caminhos de fallback de `tr()`.

**Nao tocado, deliberadamente.** `mod_preview.R` `priority = 10` (ADR-020) e `max_rows = 100L` (ADR-002), `suspendWhenHidden` e a dependencia unica de `output$map_panel` (ADR-102), `isolate()` da grade (ADR-098), gate `rendered_map_inputs` (ADR-104), `preview_cache`, debounce de idioma (ADR-053), `server = FALSE` do Wiki (ADR-042/043/083) e o cap de streaming removido pelo ADR-093. Cada um desses e uma regressao ja paga e documentada; "otimizar" qualquer um seria refaze-la.
## ADR-112: Idioma nao e insumo do dado -- `raw_data` le `lang_r()` sob `isolate()`, e trocar de idioma deixa de reler o upload

**Contexto.** Trocar o idioma da interface apagava o mapeamento inteiro: colunas mapeadas, o assistente de `basisOfRecord`, o de `establishmentMeans`/`degreeOfEstablishment`, `preview_cache` e o gate `rendered_map_inputs`. O relato do dono foi preciso: "ele trata a troca de idioma como se eu tivesse dado upload de um novo arquivo". Era literalmente isso. `raw_data` (`mod_upload.R`) e um `shiny::reactive()` que lia `lang_r()` em dez pontos, **todos** dentro de `tr()` para texto de notificacao ou de erro (inclusive os `lang =` passados a `read_camtrap_dp_zip()` / `convert_camtrap_to_dwc_occurrence()`, que so alimentam `stop(tr(...))` em `utils_camtrap.R`). Isso fazia o reativo inteiro depender do idioma: trocar de idioma o invalidava, ele **relia o arquivo do disco** e emitia um data frame novo. O `observeEvent(raw_data_r(), ...)` de `mod_mapping.R` entao fazia exatamente o que deve fazer com um upload novo: resetar tudo.

**Decisao.** `lang <- shiny::isolate(lang_r())` uma unica vez no topo de `raw_data`, e as dez chamadas passam a usar `lang`. O reativo passa a depender so de `input$file` e `file_kind()` (que por sua vez le `input$file` e `input$upload_mode`, nenhum ligado a idioma). Nenhum valor produzido muda em nenhum dos tres caminhos (CSV, guia, Camtrap DP), porque o idioma nunca entrava no dado. Efeito colateral desejavel: a notificacao de "arquivo carregado" nao e mais reemitida a cada troca de idioma; ela sai na lingua ativa no momento do upload, que e o comportamento correto para um evento pontual.

**O reset em si nao e o defeito.** O `observeEvent(raw_data_r(), ...)` esta certo: dado novo exige mapeamento novo, senao o usuario herda mapeamento de outra planilha. O defeito era a montante, no que contava como "dado novo".

**Consequencias.** Dois testes novos em `test-mod-upload-server.R`, ambos usando a mesma sonda: **trocar o conteudo do arquivo no disco entre duas leituras**. Reativo cacheado devolve a contagem antiga; reativo que releu pega a nova. O primeiro fixa que a troca de idioma nao rele (falha na `main`: 5 linhas em vez de 2); o segundo fixa que um upload de arquivo **novo** continua relendo, para o teste nao virar "nunca rele". Essa sonda e a forma generica de detectar dependencia reativa indevida em qualquer reativo que le do disco.

**Relacao com a ADR-111.** A tentativa de isolar `lang_r()` no observer de sincronia do mapeamento (ADR-111 item 5) foi **reprovada** e revertida, e o sintoma la era o mesmo (mapeamento apagado ao trocar de idioma) por uma causa **diferente** (rebind de input no DOM vs releitura do upload). Os dois casos juntos delimitam a regra: **isolar a leitura de idioma onde ela so decide texto** (aqui, correto); **nao isolar onde ela sincroniza com um `renderUI` que recria inputs** (ADR-111, errado).

## ADR-113: Segundo lote de desempenho -- carga preguicosa do reference espacial, parsers de coluna, trabalho descartado e o tick de 60ms fora do painel de configuracao

**Contexto.** Auditoria pedida pelo dono como continuacao da ADR-111 ("achar mais otimizacoes como a da 0.9.5, sem causar regressoes"). A ADR-111 corrigiu o padrao "funcao escalar chamada por registro" na camada de renderizacao; esta auditoria encontrou as **maiores** instancias do mesmo padrao fora dela (nos parsers de mapeamento e do Rostrum), mais tres classes vizinhas: custo de carga do pacote, trabalho computado e descartado, e re-render disparado por dependencia reativa desnecessaria. Quatro PRs, separadas por classe de risco: funcao pura nao mistura com cirurgia no grafo reativo, porque `testServer` nao detecta a segunda (LESSONS, ADR-111 item 5).

**Decisao.**

1. **O reference espacial deixa de ser pre-aquecido no `.onLoad()` (#73).** A linha `coords_load_ne_land(10L, download = FALSE)` custava ~5,8s de **cada** carga do pacote: ~4,4s de `loadNamespace("terra")` e ~1,4s convertendo o `sf` guardado para `SpatVector`. `terra` esta em `Imports` mas deliberadamente **fora** do `NAMESPACE` -- e `::`-only e preguicoso, como `sf`, `taxadb`, `leaflet`, `DT` e `writexl`; essa linha era a unica coisa que o forcava. A camada tem um consumidor so, `coords_load_ne_land(10L)` dentro de `validate_coords_cc_df()`, alcancado apenas pelo botao de validar coordenadas, que ja roda atras de modal bloqueante. `loadNamespace("saira")` medido em pacote instalado: **5,78s -> 1,04s**.
   Verificado que `download` nao introduz caminho de rede: para `scale = 10L` o argumento **so** controla se um `warning` e emitido (`utils_coords.R:166-177`); o RDS embutido e sempre o que se le. Download so existe para as escalas 50/110, que vao ao `rnaturalearth`.
   **O ganho nao aparece sob `pkgload::load_all()`**: o pkgload carrega todas as dependencias declaradas independentemente de o pacote toca-las (`taxadb`, `writexl` e `leaflet` ficam carregados sem ninguem os chamar no `.onLoad`). Medir com `R CMD INSTALL` numa lib temporaria + `loadNamespace()`, que e como o app implantado carrega.

2. **Quatro parsers escalares ganham companion vetorizado (#74).** Mesma forma da ADR-111, em `utils_mapping.R` e `utils_rostrum_engine.R`. Medido em 50k linhas: `parse_month_to_number` 5,85s -> 0,03s, `parse_year_to_number` 4,05s -> 0,02s, `format_genus_token` 3,24s -> 0,05s, `format_epithet_token` -> 0,04s. O de mes era o pior porque **realocava um vetor nomeado de 40 entradas a cada chamada**; a tabela virou constante de arquivo (`.month_name_map`) lida tanto pelo escalar quanto pelo companion, para as duas grafias nao poderem divergir.
   Tres armadilhas registradas porque cada uma quebraria a equivalencia em silencio: (a) digitos fora de faixa (`"0"`, `"13"`) casam `^\d{1,2}$` mas precisam **atravessar** para a busca por nome, que e o que o escalar faz -- curto-circuitar para `NA` concorda hoje e diverge no dia em que um nome de mes parecer numerico; (b) `regmatches()` **descarta** elementos sem match e encurta o vetor, desalinhando tudo depois do primeiro miss -- usar posicao de `regexpr()` + `substring()`; (c) a busca usa colchete simples, porque em **vetor nomeado** `[[chave]]` erra com chave ausente (ADR-110).
   Efeito colateral verificado e aceito: os `vapply` antigos carregavam `USE.NAMES = TRUE`, entao devolviam vetores **nomeados** e esses nomes vazavam para `valid_mask`, `partial_mask` e `failed_rows` nas listas retornadas. Os companions devolvem sem nome. Valores identicos por digest; nenhum consumidor le esses nomes e o unico acesso em teste e posicional. Era acidente do default do `vapply`, nao contrato.
   **Consertou dois budgets que ja estavam vermelhos na `main`**: Stage 2 (7,7s contra teto de 2s) e pipeline completo (10,5s contra 8s).

3. **Trabalho computado e descartado deixa de ser computado (#75).** (a) `resolve_occurrence_ids()` gerava `n` UUIDs e sobrescrevia os que o upload ja trazia -- planilha com `occurrenceID` completo (camtrap, reimportacao do GBIF) pagava por um conjunto inteiro e descartava tudo: 0,860s -> 0,017s em 50k linhas; agora dimensiona pela quantidade de lacunas, como `generate_occurrence_ids()` ja fazia em `utils_export.R`. (b) `build_eventdate_interval()` construia o texto de fallback cru sempre, embora so seja lido dentro de `if (fallback_raw && any(failed_rows))`; movido para dentro do ramo -- **15,11s -> 0,267s** em dataset limpo de 50k x 4 colunas. Em dataset com muitas falhas de parse o fallback e de fato exibido e o custo permanece (~13,4s): comportamento pretendido, inalterado. (c) `coords_transposed_corrections()` era chamado sem `country_iso3`, refazendo a cascata inteira (iso3c, iso2c, CLDR, aliases, e uma matriz `utils::adist` contra ~3k aliases) sobre a mesma coluna que `validate_coords_cc_df()` acabara de resolver e devolver.
   Duas coisas verificadas em (c) em vez de assumidas: **alinhamento** (`out` e montado com `.row_index = seq_len(n)`, toda coluna de comprimento `n` a partir de `df[[col]]`, sem subset nem reordenacao) e **equivalencia de entrada** (os dois sitios alimentam a cascata diferente -- o validador pre-limpa `NA -> ""` e `trimws`, o corretor passa a coluna crua; comparados sobre 10 grafias incluindo `"Brasill"` o resultado e `identical()`).

4. **O painel de configuracao da validacao de nomes sai do tick de 60ms (#76).** O laco de execucao reescreve `rv$run_state` a cada 60ms; `output$config_panel` o lia, entao durante uma execucao o painel inteiro -- cada card de provedor, ~30 `tr()` e os dois `bslib::input_switch` -- era reconstruido ~16 vezes por segundo, na mesma thread R unica que fazia as consultas taxonomicas. A barra passou para `output$progress_block`, o unico output que depende de `rv$run_state`.
   **Tres leituras cairam juntas, nao uma.** `snapshot <- progress_snapshot()` so era usado pelas duas linhas do bloco extraido; `report_df <- validation_result()` e `has_report` so alimentavam `show_progress_meta`; e `show_progress_meta` era atribuida e **nunca lida**. Entao o painel tambem perdeu a dependencia de `validation_result()`, isto e, parou de se reconstruir a cada revisao manual salva -- ganho que nao estava previsto.
   **Apagar a linha morta e load-bearing, nao faxina.** `reactiveValues` invalida na escrita independentemente de o valor ter mudado, entao uma leitura cujo resultado e descartado mantem a dependencia viva igual. Extrair o bloco **sem** apagar a linha nao produz efeito nenhum -- e **nenhum teste automatizado distingue as duas versoes**, a suite fica verde dos dois jeitos.
   Junto, corrigiu bug ao vivo: desligar "remover autores" ou "ignorar qualificadores" e trocar de idioma religava os dois switches, mudando em silencio como a proxima validacao trataria os nomes. O `renderUI` recria esses switches e lia o valor dos widgets que estava prestes a substituir; as duas leituras agora sao `isolate()`. Checado contra a armadilha da ADR-111 antes de isolar: nenhum observer precisa rodar no mesmo flush dessa recriacao, porque nada sincroniza esses dois inputs para dentro do `rv` -- sao lidos direto por `report_df` e `review_export_payload`.
   `stream_window()` ordenava o frame acumulado inteiro e so entao ficava com as primeiras `limit` linhas; agora ordena um indice, trunca o indice e subconjunta uma vez. Em stream de 20k nomes, 200 redraws: **1,02s -> 0,06s (17,9x)**. Contrato da ADR-093 intacto: `limit = NULL` continua devolvendo tudo e o teto so vale durante a execucao.

**Consequencias.** Budgets novos no suite `RUN_PERF`, na convencao da ADR-111 (acima do custo vetorizado, bem abaixo do custo antigo, com comentario `# Pre-ADR-113: ~Ns`). Dois deles merecem nota: o de `build_eventdate_interval` ficou **duplicado de proposito** -- um sobre fixture suja onde o fallback e de fato necessario (teto 25s, guarda contra o parser por linha voltar) e outro sobre dataset limpo (teto 2,5s, guarda contra a construcao incondicional voltar), este ultimo com `expect_identical(out$failure_count, 0L)` para a fixture nao deixar de provar nada em silencio. Testes de (3) afirmam **estrutura, nao tempo** (`ids::uuid` e `coords_country_to_iso3` mockados para erro se alcancados), cada um com o **par negativo** -- sem ele, ambos passariam tambem numa implementacao que nunca gera nem resolve nada.

**Nao tocado, deliberadamente** (alem da lista da ADR-111):
- **Reencodar `ne_land_10m_americas.rds`.** GeoPackage levaria `terra::vect()` de 1,38s para 0,027s e um `SpatVector` empacotado para ~0,04s, mas isso e 1,4s uma vez por processo dentro de uma acao explicita ja lenta, contra um artefato binario versionado regenerado. O empacotado e pior ainda: sua serializacao nao tem estabilidade garantida entre versoes do `terra` e o modo de falha e silencioso meses depois.
- **O espelho XLSX do pacote de exportacao** (2,34s contra 0,68s do CSV em 50k linhas). E funcionalidade deliberada; registrar o numero, nao remover nem tornar condicional.
- **`utils_taxadb.R:329-353`, `:454-462` e `utils_brproviders.R:1236-1247`.** Iteram sobre `query_name` **unicos**, nao sobre linhas -- o pipeline deduplica antes do taxadb por design. `n` e centenas a poucos milhares independentemente do tamanho do dataset, entao o crescimento que este lote combate nao se aplica. O `split()` + `do.call(rbind, ...)` e feio mas carrega a logica de blanking de schema em match ambiguo, sutil e bem testada.
- **Paralelizar as chamadas GBIF de IUCN/match** (`utils_threat_status.R:93`, `:131`). Ja deduplicam sobre `unique()` e memoizam por sessao, entao o laco so roda em cache miss real. Concorrencia exigiria `httr2::req_perform_parallel`, e a ADR-107 fixa contrato duro de que todo caminho de rede na fronteira de exportacao degrada em silencio para `NA`, embrulhado por fonte, sem nunca bloquear. Concorrencia multiplica as formas de violar isso, e `httr2` esta em `Suggests`.

**Pendencias abertas pela auditoria, fora do escopo deste lote:**
- **Stage 3 do Rostrum segue vermelho** (1,3-1,6s contra teto de 0,5s), pre-existente e alheio a este lote. Perfilado: tudo esta em `rostrum_apply_stage3_candidates`, onde `rostrum_column_quality_metrics(df, col_name)` custa 0,02s e **nao e memoizado**, recalculando metricas identicas uma vez por termo para a mesma coluna -- 0,994s do total numa fixture de 50 termos. Um cache por execucao com chave no nome da coluna deve resolver.
...
- **Data**: 2026-08-06
- **Status**: Aceito
- **Contexto**: A ADR-107 escreve `mmaThreatStatus`/`mmaSource` no `dynamicProperties` sob dois gates, e nenhum dos dois e geografico: `include_mma` (a pessoa marcou florabr/faunabr na aba Nomes) e o match por nome cientifico contra `sensitive_species.rds`, que nao tem coluna de pais. `include_mma` e preferencia de **fonte taxonomica**, nao afirmacao sobre localidade. Consequencia reportada: um *Panthera onca* coletado no Peru saia do export alegando categoria da Portaria MMA 1.704/2026, que e a lista vermelha **nacional brasileira** e nao cobre aquele registro.
- **Decisao**: `is_brazilian_record(df)` (`utils_export.R`) resolve o pais por linha -- `countryCode` primeiro (alpha-3 interno ou alpha-2 do publicador), `country` depois -- reusando `coords_country_to_iso3()`, a cascata de 5 camadas da ADR-036, que ja deduplica (custo por valor unico, nao por linha). No ramo `include_mma`, as duas chaves viram `NA` fora do Brasil; `merge_dynamic_property()` ja ignora `NA`, entao a linha fica intacta e o `dynamicProperties` do publicador e preservado. **Modo estrito**: linha sem pais resolvivel nao recebe as chaves -- localidade desconhecida nao e evidencia de ocorrencia no Brasil. O IUCN (`iucnRedListCategory`) e avaliacao **global** e continua indiferente a geografia; a assimetria e o ponto.
- **Consequencia de ordem**: `apply_conservation_status()` passa a rodar **depois** de `apply_country_fill_payload()` em `mod_export_download.R`, e nao mais logo apos o name review. No modo estrito, enriquecer antes do preenchimento apagaria o MMA de registros brasileiros cujo `country` so e derivado das coordenadas. Move-se o bloco de conservacao, nao o de pais, para preservar a ordem correcao-de-coordenadas -> preenchimento-de-pais. Revoga o item 5 da ADR-107 nessa parte.
- **Alternativas**:
  - Manter o MMA em linha sem pais (status quo para o indeterminado) - rejeitado pelo dono: erra para o lado de afirmar status legal onde nao ha base.
  - Fazer o proprio predicado cair em point-in-polygon quando `country` esta vazio - rejeitado: repetiria por registro o custo de `terra::extract` que a LESSONS ja registra, e o preenchimento de pais da aba Coordenadas ja produz esse dado uma vez so.
  - Tornar a contagem de resumo da aba Nomes ciente de pais - rejeitado: o report e por nome e o pais derivado de coordenada so existe no download, entao a contagem divergiria do export nos dois sentidos. Corrigido no texto (`validate_names_conservation_summary_mma` passa a dizer "especie(s)" e a declarar o escopo Brasil), nao na contagem.
- **Fora de escopo**: `mask_sensitive_coordinates()` tambem usa a lista MMA sem filtro de pais, mas e acionada explicitamente pela pessoa, por especie e com justificativa obrigatoria (ADR-100); generalizar demais e conservador, nao afirmacao falsa.

## ADR-122: coordenada UTM e convertida pela Saira, mas a zona quem escolhe e a pessoa

- **Data**: 2026-09-03
- **Status**: Aceito
- **Contexto**: Uma usuaria publicou `decimalLatitude`/`decimalLongitude` com valores UTM (`574699`, `7805441`) em vez de graus. A validacao marcava as 2011 linhas como `validity_bounds` -- correto e inutil: o conserto e uma conversao, nao uma edicao. O caso e comum em dado brasileiro de campo, onde o GPS entrega UTM.
- **Decisao**: Converter dentro da aba Coordenadas, como mais um card de correcao, reusando `merge_coords_corrections()`. A **zona nao e inferida**: a Saira pontua candidatas contra o pais informado, apresenta a lista e exige que a pessoa escolha e confira no mapa antes de aplicar. O datum tem default SIRGAS 2000 num seletor secundario.
- **Por que a zona nao pode ser deduzida**: o mesmo par easting/northing e valido em **todas** as zonas, e cada uma coloca o ponto exatamente 6 graus de longitude ao lado, na mesma latitude. Nao ha erro visivel: a zona errada produz uma coordenada plausivel. No dado real, as zonas 21, 22 e 23 puseram 100% dos pontos dentro do Brasil -- o pais nao desempata em pais largo. Errar a zona desloca o registro ~600 km, em silencio.
- **Por que o datum pode ter default**: medido no mesmo par, WGS 84 fica a **0,0 m** de SIRGAS 2000, SAD69 a 68,5 m e Corrego Alegre a 81,2 m. O datum erra por dezenas de metros, a zona por centenas de quilometros -- quatro ordens de grandeza separam os dois riscos, e so o segundo justifica travar o fluxo.
- **Alternativas**:
  - Inferir a zona e aplicar sozinha - rejeitado pelo motivo acima: o modo de falha e silencioso e plausivel, exatamente o que o motor de mapeamento evita por principio.
  - Perguntar tambem o datum, sem default - rejeitado: cobra da pessoa uma decisao que quase nunca muda o resultado.
  - Preencher `geodeticDatum` automaticamente - rejeitado: o termo descreve a saida (WGS 84 apos a conversao), ja existe em `get_dwc_full_catalog()` e a pessoa o adiciona por "Adicionar termo"; preenche-lo daqui exigiria a aba Coordenadas escrever no estado da aba Mapeamento.
- **Preservacao do original**: a conversao **cria** `verbatimLatitude`, `verbatimLongitude`, `verbatimCoordinateSystem` e `verbatimSRS` quando a planilha nao as traz, e grava o par projetado junto com a zona e o datum confirmados (`UTM zone 21S`, `EPSG:31981`). Sem isso o dado de origem se perdia: `apply_coords_correction_payload()` so preservava o verbatim **se a coluna ja existisse**, e uma planilha que publica UTM em `decimalLatitude` nao tem de onde tirar essa coluna. As outras correcoes (transposta, troca de sinal) seguem inalteradas -- elas nao enviam colunas verbatim, e para elas a preservacao continua condicionada a coluna existir. Os quatro termos resolvem para URI DwC, entao entram no `meta.xml` e sao publicados.
  - Os eixos vao para o termo que significam: northing em `verbatimLatitude`, easting em `verbatimLongitude`, **nao** para a coluna em que estavam. Na planilha reportada os dois estavam trocados, e preservar a troca produziria um verbatim que nao reconverte para o mesmo ponto.
  - Nao ha acoplamento novo: a zona e o datum vem do proprio card, nao do estado da aba Mapeamento.
- **Consequencias**: `merge_coords_corrections()` passa a alinhar os payloads pela uniao das colunas, porque o card de UTM carrega colunas que os outros nao tem. `coords_utm_zone_candidates()` desliga o `s2` para o teste ponto-em-poligono (o Natural Earth tem aneis que o motor esferico recusa) e restaura no `on.exit`, porque o ajuste e global e a aba de Generalizacao divide a mesma sessao `sf`.

## ADR-123: autoria sai do `scientificName` e vai para `scientificNameAuthorship`

- **Data**: 2026-09-03
- **Status**: Aceito
- **Contexto**: `extract_scientific_name_components()` ja precisava reconhecer autoria para nao confundi-la com epiteto infraespecifico (ADR-119), mas so a usava para descartar. Um upload com `Panthera onca (Linnaeus, 1758)` publicava o nome inteiro em `scientificName` e deixava `scientificNameAuthorship` vazio, mesmo com a informacao ali.
- **Decisao**: A autoria e extraida e publicada em `scientificNameAuthorship`, e o `scientificName` fica **so com o taxon** (`Panthera onca`). Uma coluna que a pessoa tenha mapeado para o termo vence a extraida, como nos demais termos derivados. So a linha que o parser resolveu e reescrita: o que ele nao entendeu mantem o valor original em vez de ser cortado no palpite.
- **Alternativa rejeitada pelo dono do projeto**: copiar em vez de mover, mantendo `scientificName` completo. O Darwin Core define o termo como "the full scientific name, **with authorship and date information if known**", e o GBIF usa o nome completo para casar o taxon com mais precisao, entao mover perde sinal de correspondencia e a distincao entre combinacoes homonimas. A escolha foi por nome limpo, com a autoria preservada no termo proprio -- nada e descartado, so muda de coluna.
- **Como autor e epiteto sao separados**: os dois ocupam a mesma posicao (`Panthera onca` vs `Panthera Oken, 1816`), e `format_epithet_token()` nao serve de juiz porque faz `tolower()` antes de validar (ADR-119). A regra olha o token cru: **inicial maiuscula seguida de minuscula** e autoria (sobrenome), **tudo maiusculo** continua epiteto. A segunda metade e o que protege planilha gritada em caixa alta (`PANTHERA ONCA`), comum em dado de campo, que uma regra de "minuscula estrita" quebraria.
- **Consequencias**: corrige de passagem um defeito que a extracao expunha -- `Panthera Oken, 1816` era lido como especie `oken`, porque `format_epithet_token()` aceitava `Oken,`. Agora sai como rank `genus` com a autoria no termo certo. `scientificNameAuthorship` fica fora de `selected_terms`, entao um conjunto sem nenhuma autoria nao ganha coluna vazia nem `<field>` no `meta.xml`, igual a `infraspecificEpithet`.

---

## ADR-124: o upload le toda coluna como texto -- palpite de tipo apagava valor

- **Data**: 2026-09-10
- **Status**: Aceito
- **Contexto**: `read_biodiversity_csv()` chamava `readr::read_delim()` sem `col_types`. O readr adivinha o tipo de cada coluna por amostragem e converte para `NA` todo valor que nao cabe no tipo adivinhado, sem erro. Num arquivo de 70.747 linhas com colunas de medida esparsas, a amostra caiu so em linhas vazias: tres colunas foram lidas como `logical` e tres como `double`. No export saiam 152 celulas apagadas, `LTR_DENS_HA` inteira perdida (ficou 100% `NA`, e o filtro de coluna vazia da ADR-120 a removeu do arquivo) e -- pior que apagar -- `1` publicado como `TRUE`, valor que nao existe em lugar nenhum da planilha de origem.
- **Decisao**: `col_types = readr::cols(.default = readr::col_character())` no `read_delim()` e `colClasses = "character"` no fallback `utils::read.csv()`. Toda coluna entra como texto, verbatim.
...
- **Decisao**:
  ...
  - Coordenadas e Nomes: configuracao e correcoes numa barra no topo, sem titulo de pagina; mapa e tabela ocupam a largura. Correcoes viram chips curtos com a frase completa no `title`. Legenda sobre o mapa.
  - Generalizacao: a Tabela 5 do Chapman vira uma matriz (grupo x perguntas 4.3/4.4/4.5, o primeiro Sim define a grade). Categorias ficam neutras, com um quadrado do tamanho da grade; cor fica so para o status de ameaca (MMA). Os pontos do mapa levam a cor do grupo quando decididos, brancos enquanto esperam; celulas neutras e borda vermelha tracejada quando saem do pais; contorno branco no fundo de satelite. Saem o card de resultado por especie e a tabela de referencia; a fonte vira um texto curto com o DOI.
  - Exportacao: quatro numeros, uma lista de pendencias com a acao que resolve cada uma, os arquivos com o aviso de que os metadados se completam no IPT e uma barra de download fixa no rodape.
