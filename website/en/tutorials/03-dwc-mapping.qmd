...
<figure class="shot">
  ...
  <figcaption><b>Figure 7.</b> The <b>Import mapping template</b> dialog: pick the <code>mapping_guide_*.txt</code> file that came in the <code>.zip</code> and confirm. Each line becomes an alias to auto-map future spreadsheets.</figcaption>
</figure>
```
...
Some terms describe the **whole dataset**, not each record: who holds the rights (<span class="term">rightsHolder</span>), the institution and collection (<span class="term">institutionCode</span>, <span class="term">collectionCode</span>), the country (<span class="term">country</span>), a reference or citation (<span class="term">references</span>, <span class="term">bibliographicCitation</span>), or the geodetic datum (<span class="term">geodeticDatum</span>). Instead of adding a repeated column to your spreadsheet, tick **Use a fixed value** on the term's card and type the value once — Saíra writes it to **every row** at export. The card looks exactly like a normal one until you tick the box, so nothing clutters the screen needlessly. Because mapping a column and using a fixed value are alternatives (an **OR** separates them), ticking the box hides the column selector — so the constant can never silently overwrite a column you had already mapped.
...
<figure class="shot">
  ...
  <figcaption><b>Figure 10.</b> On an eligible term's card, tick <b>Use a fixed value</b> and type the value once. <i>(placeholder — replace with a screenshot.)</i></figcaption>
</figure>
<figure class="shot">
  ...
  <figcaption><b>Figure 11.</b> The warning strip makes clear the value fills <b>every row</b> of that field. <i>(placeholder — replace with a screenshot.)</i></figcaption>
</figure>
```
