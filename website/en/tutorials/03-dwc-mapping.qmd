...
<span class="term">establishmentMeans</span> says **how the taxon got there** (native, introduced, vagrant…) and <span class="term">degreeOfEstablishment</span> says **what situation it is in** (captive, cultivated, released, invasive…). Both carry a TDWG controlled vocabulary, 7 and 11 values, and both describe the **species**, not the row. Filling that in by hand across thousands of records is not realistic.

So this assistant works **per species**, not per column value like the <span class="term">basisOfRecord</span> one. It lists the distinct species found in the column mapped to <span class="term">scientificName</span>, with a record count for each and ordered from the answer that covers the most data to the one that covers the least, and asks for both values **once per taxon**. The answer is expanded to every record of that species.

```{=html}
<figure class="shot">
  <img src="../../assets/img/5.2-card-assistente-establishment-EN.png" alt="establishmentMeans card with the assistant button, next to the degreeOfEstablishment card carrying the note that points to it">
...
```{=html}
<figure class="shot">
  <img src="../../assets/img/5.3-assistente-establishment-EN.png" alt="Assistant window listing unique species with record counts, two selectors per row and an invasive badge on listed species">
...
```{=html}
<figure class="shot">
  <img src="../../assets/img/5.4-cards-establishment-preenchidos-EN.png" alt="Both cards after saving the assistant, with the mapped-field border, an ASSISTANT badge and the note reporting how many species were answered">
  <figcaption><b>Figure 12.</b> After saving, the cards count as filled, with an <span class="term">ASSISTANT</span> badge and a note stating how many species were answered (and how many still have no degree).</figcaption>
</figure>
```
...
<figure class="shot">
  ...
  <figcaption><b>Figure 13.</b> On an eligible term's card, tick <b>Use a fixed value</b> and type the value once.</figcaption>
</figure>
<figure class="shot">
  ...
  <figcaption><b>Figure 14.</b> The warning strip makes clear the value fills <b>every row</b> of that field.</figcaption>
</figure>
```
...
```{=html}
<figure class="shot">
  <img src="../../assets/img/6.1-card-dynamicproperties-previa-EN.png" alt="dynamicProperties card with the per-column key editor and, below it, the JSON preview assembled for the first row">
  <figcaption><b>Figure 15.</b> The key editor and, underneath, the JSON assembled for the first row (for example <code>{"colour":"melanic"}</code>), refreshed on every key edit.</figcaption>
</figure>
```
