...
Precise data has **greater scientific value**. Generalizing is the exception, not the rule: reduce precision only when there is a real risk of collection, persecution or exploitation. When in doubt, consult your country's conservation-authority guidelines. In Brazil, follow GBIF's.
:::
...
- **Publish original coordinates** *(recommended)*: keeps the full precision.
...
Saíra also **detects automatically** coordinates that already arrived generalized and **does not generalize them again**, as long as the source carries the signal: <span class="term">coordinateUncertaintyInMeters</span> of 1 km or more, or a filled <span class="term">dataGeneralizations</span>. Without either of those the detection has no way to tell, and the decision becomes yours.
...
Generalizing only **reduces** precision, it never invents it. If a coordinate is already less precise than the category you picked, Saíra publishes it at the **data's real precision**, without faking 100 m, and still protects the record with <span class="term">informationWithheld</span>, <span class="term">dataGeneralizations</span> and the uncertainty radius. A banner on the tab counts how many records were published this way.
:::
