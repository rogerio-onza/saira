...
Precise data has **greater scientific value**. Generalizing is the exception, not the rule: reduce precision only when there is a real risk of collection, persecution or exploitation. When in doubt, publish the original coordinate.
:::
...
- **Assess sensitivity** — opens Chapman's decision table to determine, with no guessing, the generalization level of each threat group.
...
## The decision table (Chapman's Table 5)
...
### The four levels (Chapman's Table 7)
...
| Level | Generalization grid | Spatial result |
|---|---|---|
| **Category 1 — Extreme** | 1° (~111 km) | Country or region |
| **Category 2 — High** | 0.1° (~11 km) | State or province |
| **Category 3 — Medium** | 0.01° (~1 km) | City or district |
| **Category 4 — Low** | 0.001° (~100 m) | Local area |

: Generalization levels (Table 7, Chapman 2020 / GBIF)

::: {.callout-note}
## Generalization never increases precision
Generalizing *reduces* precision — it never invents it (Chapman 2020, §4.1). If a coordinate is already *less* precise than the category you picked — say, a coordinate from a paper with only two decimal places (~1 km) while you chose Category 4 (~100 m) — Saíra **publishes it at the data's real precision** (the grid is never finer than the original) instead of rounding to a false 100 m precision. The record is **still protected**: it gets <span class="term">informationWithheld</span>, the generalized cell, the uncertainty radius, and the locality fields scrubbed. <span class="term">dataGeneralizations</span> records **both precisions** — the one provided and the one originally retained by the custodian (Chapman §4.3 and §5.1). A banner on the tab counts how many records were published at the data's real precision.
:::
...
At ~111 km the record loses almost all usefulness for spatial analysis and distribution modeling. **Do not use it** merely to mask localities the local community already knows — reserve it for extreme cases of threat to the species.
:::
...
On generalized records, Saíra publishes the **cell coordinate** (not the original), fills <span class="term">coordinateUncertaintyInMeters</span> and <span class="term">dataGeneralizations</span>, and **blanks the fields that could reveal the exact locality** (verbatim coordinates and the like). The **original, precise coordinates** of those records do not go into the public package: they stay in a **separate file**, under your control, so you never lose the precise data.
...
- Chapman, A. D. (2020). *Current Best Practices for Generalizing Sensitive Species Occurrence Data.* Copenhagen: GBIF Secretariat. <https://doi.org/10.15468/doc-5jp4-5g10> — Table 5 (decision cascade) and Table 7 (generalization grid).
