...
If you did not map an <span class="term">occurrenceID</span>, Saíra warns you: the IPT/GBIF then generate unstable identifiers on each publish. Export still works, and the identifiers Saíra generates are derived from the row content, so re-uploading the same spreadsheet reproduces them. Even so, the recommendation is to **map the column that already carries your identifiers**: Saíra keeps the ones you have and fills only the blank rows.
:::
...
- **Dates** converted to ISO format (`YYYY-MM-DD`), whatever separator the spreadsheet used. A date with no day comes out as `YYYY-MM`, and a date carrying a time as `YYYY-MM-DDThh:mm`. If the year falls outside 1600 and the current year, Saíra warns and names the rows: the error is a typo and only you know what the date should be. The warning does not block the export.
- **Coordinates** with the decimal separator normalized (comma becomes dot).
- **occurrenceID** derived from the row content when missing, to guarantee a unique, stable identifier per record.
...
::: {.callout-warning}
## The MMA status is national
The MMA category comes from Brazil's red list, so Saíra writes it only to records whose `country` (or `countryCode`) resolves to Brazil. In a dataset spanning several countries, the same species gets the MMA keys on the Brazilian row and not on the foreign one. A record with no country is left without the keys too: map `country`, or use the fill from coordinates in [coordinate validation](05-coordinate-validation.qmd). `iucnRedListCategory` is global and does not depend on this.
:::
