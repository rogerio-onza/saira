...
To practice this and the next steps, download [`ocorrencias-demo.csv`](../../assets/exemplo/ocorrencias-demo.csv), Saíra's demonstration dataset: 1,107 occurrences of Brazilian species across several biomes, deliberately imperfect. It has 25 columns to map, day, month and year in separate columns, one date column mixing formats (slashes, times, month and year only) and two impossible years, catalogue numbers missing and repeated, coordinates swapped and out of range, records from neighbouring countries, threatened species for generalization and invasive alien species. Every tutorial step has something to resolve in it.
:::

In the **Home** step, drag the file onto the indicated area or click to select it.
...
```{=html}
<figure class="shot">
  <img src="../../assets/img/2-area-upload-resumo-arquivo-EN.png" alt="Saíra upload panel with the file summary: 1,107 records, 25 columns, 0.3 MB">
