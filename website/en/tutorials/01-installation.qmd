...
Because Saíra **is not on CRAN**, it is installed from source. On **Windows**, compiling packages from source requires **Rtools**. Install the Rtools version that matches your R version before installing Saíra. (On macOS and Linux this step is usually not needed.)

::: {.callout-note}
## Rtools lives on the same R page
No need to look elsewhere: the Rtools installer is **on the same R download page on CRAN** (Figure 5 shows the link). Download the Rtools version that matches your R right there.
:::
...
Saíra is installed from GitHub, so **updating means reinstalling**: the same install command always pulls the latest version.

1. Close the app, if it is open.
2. In the R (or RStudio) console, run again:

```r
pak::pak("rogerio-onza/saira")
```

`pak` downloads the latest version and updates only what changed (later installs are fast). Reopen the app with `saira::run_app()`. To check which version is installed, run `packageVersion("saira")`.
