...
::: {.tut-standfirst}
Coordenadas invertidas, trocadas ou em (0, 0) estão entre os erros mais comuns em planilhas de campo. O Saíra roda um diagnóstico determinístico, mostra cada registro no mapa e numa tabela, e **propõe correções seguras**, aplicadas só na exportação, nunca por cima dos seus dados.
:::
...
A barra no topo da tela mostra a coluna associada a cada termo. Enquanto algum faltar, o botão **Validar Coordenadas** fica desabilitado.

A validação roda sempre todos os testes, incluindo a detecção de *referências sensíveis* (pontos suspeitos sobre capitais, centróides de país, sede do GBIF e instituições de biodiversidade).

Clique em **Validar Coordenadas**. A validação é disparada por você, não a cada mudança nos dados.

```{=html}
<figure class="shot">
  <img src="../assets/img/9-painel-validação.png" alt="Barra da validação de coordenadas: status das colunas mapeadas (latitude, longitude, country) e o botão Validar Coordenadas">
  <figcaption><b>Figura 1.</b> Barra no topo: status das colunas mapeadas (latitude, longitude, country) e o botão Validar Coordenadas.</figcaption>
</figure>
```
...
## Corrigir uma coordenada na tabela

Quando nenhum painel cobre o erro, corrija o ponto na própria tabela **Diagnóstico de Coordenadas**, sem voltar à planilha:

```{=html}
<ol class="fix-steps">
  <li><strong>Clique duas vezes na latitude ou na longitude.</strong>
    <p>A célula vira um campo. Digite o valor em graus decimais, com vírgula ou ponto (<code>-15,8</code> ou <code>-15.8</code>). Enter salva.</p></li>
  <li><strong>Veja o novo diagnóstico.</strong>
    <p>O Saíra valida o ponto de novo, com os mesmos testes. Se ele passa, a linha fica <em>Editada</em> e <em>Corrigida</em>. Se ainda falha (por exemplo, continua no mar), a linha mostra o problema novo. O marcador se move no mapa.</p></li>
  <li><strong>Desfaça se precisar.</strong>
    <p>O link <strong>Desfazer</strong> da linha devolve a coordenada original. O filtro <strong>Editadas</strong> mostra só as linhas que você corrigiu.</p></li>
</ol>
```

```{=html}
<figure class="shot">
  <img src="../assets/img/9.5-editar-coordenada-tabela.png" alt="Tabela Diagnóstico de Coordenadas com a primeira linha editada: selos Editada e Corrigida, o link Desfazer e a nova longitude">
  <figcaption><b>Figura 6.</b> Uma linha corrigida na tabela: selos <em>Editada</em> e <em>Corrigida</em>, o link Desfazer e o novo valor.</figcaption>
</figure>
```

A correção manual segue as regras das assistidas: entra na exportação, e o valor original vai para <span class="term">verbatimLatitude</span> e <span class="term">verbatimLongitude</span> quando essas colunas estão vazias. Validar de novo não apaga as correções manuais. Uma linha sem <span class="term">occurrenceID</span> único não pode ser editada.

**Registros sem coordenada** ficam marcados como *Coordenada ausente*. O Saíra não inventa um ponto a partir do texto da localidade: esses registros ficam a seu critério. Mantenha-os sem georreferenciamento de forma consciente, ou digite as duas coordenadas na tabela.

::: {.callout-tip}
## Datum e precisão
Registre o <span class="term">geodeticDatum</span> (por exemplo, <em>WGS84</em>) e, quando souber, a incerteza em metros (<span class="term">coordinateUncertaintyInMeters</span>). Sem esses dois campos, quem reutiliza o registro não consegue saber se o ponto é confiável a 10 m ou a 10 km. Muitas análises, como modelagem de distribuição, filtram justamente por <span class="term">coordinateUncertaintyInMeters</span>.
:::
