...
Como o Saíra **não está no CRAN**, ele é instalado a partir do código-fonte. No **Windows**, compilar pacotes a partir do código exige o **Rtools**. Instale a versão de Rtools que corresponde à sua versão do R antes de instalar o Saíra. (No macOS e no Linux esse passo normalmente não é necessário.)

::: {.callout-note}
## O Rtools fica na mesma página do R
Você não precisa procurar em outro site: o instalador do Rtools está **na mesma página de download do R no CRAN** (a Figura 5 mostra o link). Baixe a versão de Rtools compatível com o seu R ali mesmo.
:::
...
O Saíra é instalado a partir do GitHub, então **atualizar é reinstalar**: o mesmo comando da instalação traz sempre a versão mais recente.

1. Feche o app, se estiver aberto.
2. No console do R (ou do RStudio), rode novamente:

```r
pak::pak("rogerio-onza/saira")
```

O `pak` baixa a versão mais recente e atualiza só o que mudou (as instalações seguintes são rápidas). Reabra o app com `saira::run_app()`. Para conferir qual versão está instalada, rode `packageVersion("saira")`.
