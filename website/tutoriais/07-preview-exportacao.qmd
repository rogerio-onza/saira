...
Só nesse caso marque **apenas o GBIF** na validação de nomes: o Saíra grava só `iucnRedListCategory` (IUCN global) e não aplica a categoria MMA, que vale para a fauna e a flora do Brasil. Se você trabalha com espécies brasileiras, mantenha a base nacional marcada.
:::
...
Os metadados (`eml.xml`) são montados **automaticamente**: o Saíra usa um título padrão, calcula a extensão geográfica e o intervalo de datas a partir dos seus dados e aplica a licença CC0 por padrão — você não precisa preencher nada. O ajuste fino dos metadados (instituição, responsáveis, licença definitiva) é feito depois, no IPT, na hora de publicar.
...
O `.zip` também traz um **modelo de mapeamento** (`<dataset>-mapping-guide.txt`): o registro "sua coluna → termo Darwin Core", sem dados. Guarde-o para auto-mapear planilhas futuras com a mesma estrutura, via **Importar modelo** (ver [Mapeando Darwin Core](03-mapeamento-dwc.qmd)).

Além do DwC-A, o Saíra exporta o conjunto como `.xlsx` com **todas as células forçadas a texto**, evitando que o Excel reinterprete datas, números de tombo ou coordenadas — útil para conferência manual ou para enviar a colaboradores.
