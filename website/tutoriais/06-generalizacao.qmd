...
Dados precisos têm **maior valor científico**. Generalizar é a exceção, não a regra: reduza a precisão apenas quando houver risco real de coleta, perseguição ou exploração. Na dúvida, publique a coordenada original.
:::
...
- **Publicar coordenadas originais** *(recomendado)* — mantém a precisão total. É a escolha certa na maioria dos casos.
- **Avaliar sensibilidade** — abre a tabela de decisão de Chapman para determinar, sem chute, o nível de generalização de cada grupo de ameaça.
...
## A tabela de decisão (Tabela 5 de Chapman)
...
### Os quatro níveis (Tabela 7 de Chapman)
...
| Nível | Grade aplicada | Resultado espacial |
|---|---|---|
| **Categoria 1 — Extrema** | 1° (~111 km) | País ou região |
| **Categoria 2 — Alta** | 0,1° (~11 km) | Estado ou província |
| **Categoria 3 — Média** | 0,01° (~1 km) | Cidade ou distrito |
| **Categoria 4 — Baixa** | 0,001° (~100 m) | Área local |

: Níveis de generalização (Tabela 7, Chapman 2020 / GBIF)

::: {.callout-note}
## A generalização nunca aumenta a precisão
Generalizar **reduz** a precisão — nunca a inventa (Chapman 2020, seç. 4.1). Se a coordenada já vier **menos precisa** que a categoria escolhida — por exemplo, uma coordenada de artigo com apenas duas casas decimais (~1 km) e você escolheu a Categoria 4 (~100 m) — o Saíra **publica na precisão real do dado** (a grade nunca fica mais fina que o original), em vez de arredondar para uma falsa precisão de 100 m. O registro **continua protegido**: recebe <span class="term">informationWithheld</span>, a célula generalizada, o raio de incerteza e a limpeza dos campos de localidade. O <span class="term">dataGeneralizations</span> grava **as duas precisões** — a fornecida e a originalmente retida pelo custodiante (Chapman seç. 4.3 e 5.1). Um aviso na própria aba conta quantos registros saíram na precisão real do dado.
:::
...
A ~111 km o registro perde quase toda a utilidade para análise espacial e modelagem de distribuição. **Não a use** apenas para mascarar localidades que a comunidade local já conhece — reserve-a para casos extremos de ameaça à espécie.
:::
...
Nos registros generalizados, o Saíra publica a **coordenada da célula** (não a original), preenche <span class="term">coordinateUncertaintyInMeters</span> e <span class="term">dataGeneralizations</span> e **apaga os campos que poderiam revelar a localidade exata** (coordenadas verbatim e similares). As **coordenadas originais e precisas** desses registros não vão para o pacote público: ficam num **arquivo à parte**, sob seu controle, para que você não perca o dado preciso.
...
- Chapman, A. D. (2020). *Current Best Practices for Generalizing Sensitive Species Occurrence Data.* Copenhagen: GBIF Secretariat. <https://doi.org/10.15468/doc-5jp4-5g10> — Tabela 5 (cascata de decisão) e Tabela 7 (grade de generalização).
...
Decisões de sensibilidade tomadas, é hora de fechar o pacote: siga para **[Pré-visualização e exportação](07-preview-exportacao.qmd)**.
