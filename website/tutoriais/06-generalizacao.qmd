...
Dados precisos têm **maior valor científico**. Generalizar é a exceção, não a regra: reduza a precisão apenas quando houver risco real de coleta, perseguição ou exploração. Na dúvida, consulte as diretrizes das autoridades de conservação do seu país. No Brasil, recomenda-se seguir as diretrizes recomendadas pelo GBIF.
:::
...
O Saíra também **identifica automaticamente** coordenadas que já chegaram generalizadas (de uma fonte anterior) e **não as generaliza de novo** — assim a localização nunca é degradada duas vezes.
...
A ~111 km o registro perde quase toda a utilidade para análise espacial e modelagem de distribuição. **Não a use** apenas de forma indiscriminada e sem justificativas plausíveis.
:::
